...
- **§6 분기 예측 (2026-08-09 실측 확인)** — 머지 후 정합에서 rebase 충돌 0건 · apply가 삭제한 후보 디렉토리 부활 0건. PR #90 apply에서 신규 야간 커밋 0건이 확인돼 `reset --hard main` 경로를 탔고, 무손실 증명(`diff $MERGED main`)이 main 우위 1건만 보여 예측대로 충돌·부활 모두 0이었다. **rebase 경로(야간 커밋 있음)는 아직 미측정** — 다음에 그 분기를 타는 주에 확인하고 이 항목을 지운다.
