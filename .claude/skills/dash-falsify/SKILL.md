...
2. PR 본문 조립 — **타깃별 섹션**. **링크는 반드시 클릭 가능한 GitHub URL로 생성** — evolve/dash 브랜치 파일은 PR 화면에서 경로 텍스트로는 안 보이므로 판정/스크린샷을 사람이 못 본다. 형식: DECISION·개별 PNG = `https://github.com/<owner>/<repo>/blob/evolve/dash/<경로>`, 샷 폴더 = `https://github.com/<owner>/<repo>/tree/evolve/dash/<경로>` (owner/repo는 `gh repo view --json owner,name`로 확인). 라운드별로 세 링크를 표에 넣는다: [DECISION](blob DECISION.md) · [승자 대표샷](blob `shots/<승자v>-1440.png`, 모바일 타깃은 `-390.png`) · [전체샷](tree `shots/` 폴더).
   ...
   - `## 라운드 예산 준수`: `node scripts/round-budget.mjs --explain 1` 로 현재 허용치를 얻고, **이번 기간에 하루 2라운드 이상 돈 날이 있는지** 원장에서 센다. 허용치가 1인데 2건 이상 돈 날이 있으면 **finding 으로 올린다** — 그 라운드 DECISION 에 "연속의 N라운드째" 가 적혀 있으면 근거로 인용한다.
     **왜 사후 탐지가 필요한가**: 예산은 `/dash-evolve` §0-0-1 이 스크립트로 계산하지만, 그 스크립트를 **호출했는지**는 실행 로그에만 남는다. 2026-09-12 에 규칙을 산문으로 적었을 때 바로 다음 실행이 어겼고(`auto-dash-r26`) **아무 경고도 없었다** — 주간 반증이 그 창을 닫는다.
...
5-0. **스크린샷 정리 (필수)** — `node scripts/prune-shots.mjs --apply` 를 돌려, 이번에 머지하는 라운드들의 프레임을 **대표 프레임만 남기고** 지운다. 16프레임은 **판정하는 동안** 필요하고(judge 가 스크롤 지점별 결함을 인용한다) 판정이 끝나면 주간 PR 이 거는 대표 프레임만 계속 참조된다.
   **왜 절차인가**: 정리 규칙이 없던 동안 **2,283장 · 266MB** 가 쌓였고, Vercel 이 매 빌드에 받는 depth=1 트리 276MB 의 **96%** 가 이것이었다(2026-09-02 실측 · 그날 한 번에 274MB → 37MB). 라운드마다 2~3MB 씩 늘기 때문에 **한 번 치우는 것으로는 끝나지 않는다.**
   스크립트는 승자 대표 프레임 · `DECISION.md` 가 경로로 인용한 프레임 · 비표준 증거샷(`AB-tabular-nums.png` 류)을 보존하고, **라운드 하나라도 대표샷이 안 남으면 비-0 으로 종료**한다. `40-commissions/` 는 건드리지 않는다.
