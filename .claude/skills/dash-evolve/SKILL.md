---
name: dash-evolve
description: 자율 진화 라운드 N회 (기본 1 · 상한은 미채움 타입 큐가 정한다 — 큐가 비면 N=1 강제 · 다중 타깃 — works.ts PAGE_TYPES 순서대로 카탈로그에 없는 페이지 타입을 우선 생성하고, 전부 차면 대시보드·랜딩·네이티브 중 무작위) — 정본 brief+격리 delta로 후보 3개 생성 → 하드게이트(gate.mjs --target web: 정적·sweep·a11y·perf) → 3렌즈 judge 다수결 → delta 격리 적재 → 정제 게이트 → evolve/dash 커밋. 인자로 라운드 수(기본 1)를 받는다 — `/dash-evolve 2`. 미채움 큐가 0종이면 무엇을 줘도 1라운드만 돈다. "/dash-evolve", "자율 라운드" 시 사용. 무인 실행 전제 — 사람 확인 없이 완주하며 no-winner 라운드를 허용한다.
---

# dash-evolve — 자율 라운드 (무인, 이중 타깃)

인자: 실행할 라운드 수 `N` (기본 1). **상한은 고정 숫자가 아니라 조건이다 — 아래 참조.**
...
**상한은 미채움 큐가 정한다 (2026-09-12 개정).**

- **미채움 타입이 0종이면 `N`은 1이다.** `N≥2` 를 받아도 1라운드만 돌고 그 사실을 로그에 남긴다.
- **미채움 타입이 있으면 `N ≤ min(2, 미채움 수)`.** 상한 3은 폐지한다.

**왜 숫자가 아니라 조건인가**: `N≥2` 의 근거는 처음부터 **커버리지 하나**였다 — 위 "타깃" 항목이 적어 둔 그대로, *"연속 실행의 목적이 정확히 커버리지다"*. 2026-08 초에는 미채움이 8종이었고 한 번에 두 타입을 도는 값어치가 분명했다. **2026-09-12 현재 `PAGE_TYPES` 18종이 전부 채워져 미채움은 0종이다 — 그 근거가 만료됐다.** 숫자를 깎으면 새 타입이 추가될 때 다시 손으로 올려야 하고, 조건으로 묶으면 그때 자동으로 돌아온다.

**생산 속도 실측이 개정 이유다** (2026-09-12): 42일 66라운드 = **주 11회**, 라운드가 돈 날의 **평균 2.06건**(32일 중 28일이 2건 이상) — 상한 3은 사실상 안 걸렸고 **야간이 늘 N=2 로 돈 것**이 실질 변수였다. 최근 13일만 봐도 주 6.5회다.
그 속도가 리뷰 주기를 앞질렀다: PR #159 는 **2라운드로 열려 7라운드로 머지**됐고, 본문을 두 번 갱신하는 동안 매번 새 라운드가 들어왔다. §6 의 "머지 전 범위 확인"이 매번 발동했다는 것은 **아무도 안 본 채 머지될 뻔했다**는 뜻이다. 라운드를 더 만드는 것보다 **만든 것을 판정에 태우는 것**이 지금의 병목이다.
...
**제일 먼저 `Agent` 도구가 실제로 있는지 확인한다.** 없으면 §1 로 넘어가지 말고 **no-round 로 종료**하고 그 사실을 로그에 남긴다. 후보를 만들지도, 판정하지도 않는다.

**왜 중단이 진행보다 나은가**: 이 루프의 판정은 **블라인드·독립**을 전제한다 — 렌즈 3개가 서로를 모른 채 같은 산출물을 보고 만드는 다수결이라야 다수결이다. 도구가 없어 **한 세션이 후보를 만들고 그 후보를 판정하면 다수결이 자기 확인이 된다.** 3파전 동률·tie-break 같은 이 루프의 주요 관측도 전부 그 전제 위에 서 있다.

**2026-08-27 에 실제로 났다.** `auto-dash-r20`·`r21` 두 라운드가 그 상태로 완주했다. 두 DECISION 이 첫 절에 편차를 **자진 고지한 것은 옳았지만**, 고지가 판정을 유효하게 만들지는 않는다 — 2026-08-29 apply 에서 **승격 보류**로 처분됐고(후보를 드롭하지도 못했다: 판정이 무효면 승자도 패자도 그 판정으로 처분할 수 없다), 라운드 두 개분의 생성·게이트·판정 비용이 delta 만 남기고 소모됐다. **무효 산출물을 만드는 것보다 라운드를 거르는 것이 싸다.**

**그래도 돌아버렸다면**: DECISION 첫 절에 편차를 고지하고(이 규율은 유지한다), 원장 entry 에 `"self_judged": true` 를 넣는다. `/dash-falsify` 는 그 표시가 있는 라운드를 **승격 후보에서 제외**하고 delta 만 검토한다. ([[questions-queue]] Q47)

**간헐적이다** — 같은 주의 `r16`·`r17` 은 정상이었다. 그래서 "한 번 확인했으니 됐다"가 아니라 **매 라운드 확인한다.**
...
- **`vault/00-principles/page-brief-repo.md` 전문 — 웹 타깃이면 항상**. 코어가 *"어느 레포에서나 참인 규칙"* 만 담기 때문에, **이 레포에 묶이는 규칙**(카피 언어·폰트 변수명·빌드 명령·경로 레이아웃)은 전부 여기 있다. 그중 §1 **"카피 언어: 영문 전용 — 한글·혼합 금지"** 는 **기계로 강제되지 않는다**(`static-check` 에 한글 검사가 없다). 렌즈1 이 대조해야 하는데, designer 가 규칙을 못 봤으면 렌즈1 은 **세 후보를 나란히 위반시킨 채** 순위만 매긴다.
  **2026-08-26 에 실제로 물렸다**: `auto-dash-r18` 세 후보가 **전원 한글 카피**로 나왔다(승자 `c` 는 한글 1754자). 이 문서가 이 목록에 없어 designer 가 규칙을 본 적이 없고, 그전 라운드들이 영문이었던 것은 **오케스트레이터가 프롬프트에 손으로 적었기 때문**이지 정본이 전달돼서가 아니었다 — 야간 데몬은 그러지 않았다. 승자는 승격 불가 판정을 받아 **형태만 재배정 대상으로 남았다.** 링크만 두면 로드 못 한다는 위 교훈이 **또 다른 정본에서 그대로 반복됐다.**
- **`vault/00-principles/reassign-queue.md` "대기 중" — 이 타깃 항목이 있으면 §2 에서 후보 하나에 배정한다.** 탈락 사유가 **형태가 아니라 규칙 위반**이었던 후보의 형태가 여기 쌓인다. 항목마다 적힌 **탈락 사유를 배정문에 그대로 넣는다** — 안 넣으면 같은 데서 또 떨어진다. 한 라운드 최대 1개(3후보 중 하나까지).
  **이 파일이 없던 동안 2연속 놓쳤다** — `landing r14/c` 는 `r15` 가, `dash r18/c` 는 `r19` 가 그냥 지나갔다. [[curation-criteria]] 가 "탈락한 형태는 재배정 대상"이라고 세워 두었지만 **그 결정을 읽는 단계가 없었다.** 규칙이 산문에만 있으면 없는 것과 같다는 실패가 바로 위 `page-brief-repo` 항목과 **같은 주에 두 번** 났다.
...
### designer 에게는 **조립한 브리프 전문**을 넘긴다 (경로를 주지 마라)

**§1 RETRIEVE 는 오케스트레이터가 하는 일이고, 그 결과를 designer 프롬프트 본문에 옮겨 적는다.** 정본 경로만 주고 알아서 읽으라고 하지 마라 — designer 가 읽느라 예산을 다 쓴다.

**실측 (2026-08-18, `auto-landing-r11`)**: 정본 경로 4개 + 중복 금지 목록 8작품을 함께 줬더니 **3후보 중 2개가 파일을 0개 만든 채 예산이 끊겼다**(도구 호출 31·35회). 원인은 정본이 아니다 — 정본 4종 합계가 384줄뿐이다. **중복 금지 목록이 designer 를 기존 작품 8개의 `page.tsx`(각각 수백 줄)로 보냈다.** 조립한 브리프를 `SendMessage` 로 다시 넘겨 셋 다 완주시켰다.

[[page-commission]] 은 같은 함정을 먼저 겪고 이렇게 적어 뒀다 — *"designer 에게 조립한 브리프 전문을 넘긴다. 정본 경로만 주고 알아서 읽으라고 하지 않는다."* 그 교훈이 여기 없었다.

프롬프트에 **반드시 넣을 것**:
- **정본 발췌 본문** — 보이스·레이아웃·색 토큰·타이포·구조 기본형·금지(anti-slop)·에셋/인터랙션. 링크가 아니라 문장으로
- **`page-brief-core` 의 기계 검증 규칙 본문** — 결정론·폰트 웨이트 세는 법·`fontFamily` 리터럴·포커스 죽은 관용구 2종·이미지 규율·전 폭 오버플로
- **`page-brief-repo` §1 카피 언어 — 한 문장으로 못박는다**: *"화면 텍스트·헤딩·라벨·브랜드·더미 데이터 전부 영어. 한글·혼합 금지."* **이것을 빠뜨리면 라운드 전체가 한글로 나온다** — `auto-dash-r18` 실측(3후보 전원). 기계 검사가 없어서 게이트도 judge 도 못 잡고, 승격 단계에서야 드러나 그때는 고칠 수 있는 범위를 넘는다(정본 §3-1 은 순위를 바꿀 만한 수정을 금지하는데, 카피 전량 번역은 a11y detail·readout 문자열까지 다시 쓰는 일이다). native 는 `GENERATION.md` §3 이 같은 규칙을 자체적으로 담고 있어 이 항목이 불필요하다
- **열린 delta 본문** — 해당 타깃의 미종결 delta 를 요약이 아니라 지시로
- **레포 관례** — `"use client"` · Tailwind v4 유틸리티 · `next/image` · `framer-motion` import 경로 · 루트 래퍼. **designer 가 관례를 찾으러 다니지 않게 한다**
- **중복 금지는 목록 + 한 줄 설명까지만.** 기존 작품의 형태를 한 구절로 적어 주고 **"그 작품들을 열지 마라"를 명시**한다

프롬프트에 **넣지 말 것**:
- **`tsc`·게이트 실행 지시** — designer 에이전트에는 **Bash 가 없다**(`Read·Grep·Glob·Skill·Write·Edit`). 시키면 못 하는 것을 정직하게 보고하느라 예산만 태운다(2026-08-18 실측: 3designer 전원). 타입 검사는 §3 게이트 1단계가 실측한다

- **재배정 큐를 먼저 소비한다.** §1 에서 읽은 `reassign-queue.md` "대기 중"에 이 타깃 항목이 있으면, **후보 하나의 형태를 그 항목으로 고정**하고 배정문에 **탈락 사유를 그대로 적는다**(예: *"…형태. **영문 카피로. 폰트 웨이트 3종.**"*). 나머지 두 후보만 자유 탐색이다. 라운드가 끝나면 결과와 무관하게 그 항목을 **아카이브로 옮긴다** — 큐에 무한 잔류시키지 않는다.
- designer(또는 frontend-design-specialist) 에이전트 3개 병렬 호출. 각자에게: **위 조립 브리프** + 서로 다른 아키타입/형태 명시 지정(중복 금지 목록 포함) + 산출 경로.
...
- 스크린샷: 후보별로 `node scripts/capture-shots.mjs --route <라우트> --name <v> --out vault/20-generations/<run>/shots` 실행. 4폭(1280/1440/1920/390) × 4 스크롤 지점(0·35·70·100%)을 찍고, 찍기 전에 **스크롤 스루 패스**를 돌려 `whileInView` 계열 리빌을 발동시킨다. 파일명은 스크롤 0 프레임이 기존과 동일한 `<v>-<w>.png`, 나머지가 `<v>-<w>-s35.png` 식이라 기존 DECISION·PR 링크가 그대로 유효하다.
  ...
  - **프레임 예산 — judge에게 16장을 다 넘기지 마라. 후보당 3~4장으로 골라 지정한다.** 권장 조합: `<v>-1440.png`(스크롤 0) · 중간 스크롤 1장 · 마지막 스크롤 1장 · `<v>-390.png`. 모바일 전용 결함이 의심되면 390의 스크롤 지점을 하나 더 준다.
    **근본 원인은 `maxTurns`다** — `comparator` 에이전트 정의(`~/.claude/agents/comparator.md`)의 `maxTurns: 10`이 전 에이전트 중 최저다(designer 25 · developer 30 · planner 20). 두 라운드에 걸쳐 여섯 judge의 **첫 중단이 전부 도구 호출 17~20회**에서 일어났고(≈10턴, 턴당 2호출), 그때 토큰은 27k~131k로 편차가 컸다 — **일정한 것은 호출 수뿐이었다.**
    프레임을 줄이면 완주하는 이유는 예산이 늘어서가 아니라 **10턴 안에 끝낼 수 있는 일이 되어서**다. 그래서 매수 제한과 함께 **병렬 배치 읽기를 지시한다** — "한 응답에 여러 Read를 동시에 넣어라. 하나씩 읽으면 판정 전에 턴이 소진된다." 캡처 매수와 judge 입력 매수는 여전히 다른 문제이고(전수 캡처는 사후 검증·PR 링크용), 거기에 **턴 예산**이 하나 더 있다.
...
- **레벨 재책정**: curation-criteria.md 체크리스트로 판정. 재현(2라운드+)·기계 검증 가능 delta는 `{...원본, level:'L2'|'L3', supersedes:'<원본 round>', status:'provisional'}` append (원줄 수정 금지).
  - **delta 본문이 재현을 주장하면 그것이 1차 증거다.** §5에서 적은 delta 텍스트에 "**r<N> 델타를 확장한다**"·"**같은 결함이다**"·"재현"류 표현이 있으면, 정제 게이트는 그 주장을 **먼저 검증하고 결론을 명시**해야 한다 — 채택하면 L2를 append하고, 기각하면 **왜 같은 결함이 아닌지**를 DECISION에 적는다. 침묵하거나 반대 결론을 근거 없이 내면 안 된다.
    근거: `auto-contact-r2`에서 같은 라운드의 두 산출물이 **정면으로 모순**했다. delta 본문은 *"r1 델타를 확장한다 … 같은 결함이다"*라고 적었는데, 정제 게이트는 *"서로 다른 구체적 축이라 … 재현 2라운드 임계 미달, L2 승격 보류"*로 반대 결론을 냈다. 둘 다 그 라운드가 쓴 것이고 아무도 충돌을 알아채지 못해, `contact`는 **2라운드를 돌고도 L2가 0건인 첫 타깃**이 됐다(L2가 있는 6개 타깃은 전부 2라운드 이상 돈 것들이다 — 누적 L2 22·L3 4로 메커니즘 자체는 정상 작동해 왔다). 2026-08-11에 사람 판단으로 delta 본문 쪽을 채택해 L2를 소급 적재했다.
  - **재현 판정은 구체 기법이 아니라 결함 클래스로 한다.** 두 delta의 **기법이 다르다는 것**(평문 마크업 vs 조건부 렌더)은 다른 결함이라는 근거가 못 된다 — 같은 증명이 같은 방식으로 도달 불가능해지면 경로가 달라도 같은 결함이다. 이미 [[curation-criteria]] Q6가 같은 원리를 말한다(스킨 차이를 눈으로 가르지 말고 **입력축 × 출력축 조합표**로 환원). 반대 방향의 선례도 있다 — [[brief-profile]] §2는 *"위반이 재발해서가 아니라 패턴이 유지돼서"* 승격된 delta를 명시한다.
