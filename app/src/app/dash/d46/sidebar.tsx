...
import { Check, ChevronsUpDown, SlidersHorizontal, Wrench, X } from "lucide-react";
  ...
  return (
    <div ref={ref} className="relative px-3 pt-3">
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className={cx("flex h-11 w-full items-center gap-2 rounded-xl border px-2.5 text-left", BORDER, "bg-white", HOVER_ACTIVE_BG, TRANSITION, FOCUS_RING)}
      >
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-11 shrink-0 items-center gap-2 px-4">
        <span className={cx("grid h-7 w-7 shrink-0 place-items-center rounded-[8px] text-white", "bg-amber-700")}>
          <Wrench size={16} aria-hidden="true" strokeWidth={2.25} />
        </span>
                  ...
                  return (
                    <li key={item.id}>
                      <span aria-disabled="true" className={cx("flex min-h-11 cursor-not-allowed items-center gap-3 rounded-lg px-2.5 py-2 text-sm font-medium", TEXT_CAPTION)}>
                        ...
                        {item.label}
                        <span className={cx("ml-auto rounded-full px-1.5 py-0.5 text-[10px] font-medium", "bg-zinc-100", TEXT_CAPTION)}>Soon</span>
                      </span>
                    </li>
                  );
                }
