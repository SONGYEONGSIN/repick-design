...
export function SlaComplianceCard({ period }: { period: Period }) {
  ...
  return (
    <Card as="section" id="sla-card" aria-labelledby={TITLE_ID} className="flex flex-col">
      <CardHeader icon={<BarChart3 className="h-4 w-4" aria-hidden="true" />} title="SLA compliance by priority" titleId={TITLE_ID} />
      ...
      <div className="px-4 py-4 sm:px-5">
        <div className="mb-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-zinc-400">
          ...
          <span className="ml-auto whitespace-nowrap text-zinc-400">{stat.shortLabel} · All channels</span>
        </div>
              ...
              <div key={row.priority}>
                <div className="mb-1 flex items-center justify-between text-[13px]">
                  ...
                  <span className="tabular-nums text-zinc-400">
                    <span className="font-semibold text-zinc-100">{formatPercent(Math.round(withinPct))}</span> compliant ·{" "}
                    {formatNumber(total)} tickets
                  </span>
                </div>
                <div className="flex h-2 w-full overflow-hidden rounded-full bg-white/5" role="img" aria-label={`${meta.label} SLA: compliant ${Math.round(withinPct)}%, at risk ${Math.round(atRiskPct)}%, breached ${Math.round(breachedPct)}%`}>
