/**
 * Palisade — route-scoped design tokens. Class constants used only within this route, without
 * touching the global theme.
 * Theme = B2B Service palette (colors.catalog): light default is a pure-white/zinc-50 canvas
 * (never cream/paper), navy-adjacent neutral text, `dark:` variants use zinc-950/900 surfaces with
 * white/10 borders. Single-accent principle: sky-700/sky-400 (#0369A1 family) is the ONLY color
 * used for emphasis (buttons, active states, focus rings, "allowed" cells); every other surface is
 * neutral zinc/slate. Contrast rule (checked in every state, not just default render): secondary/
 * caption text is never lighter than zinc-500 on light surfaces, never darker than zinc-400 on dark
 * surfaces — this includes "denied"/"no access" cells only reachable via toggle or filter states.
 */
...
export const ACCENT_SOLID = "bg-sky-700 text-white hover:bg-sky-600 active:bg-sky-800 dark:bg-sky-500 dark:text-zinc-950 dark:hover:bg-sky-400 dark:active:bg-sky-600";
...
/* Status/tone — used on audit action badges. Always paired with an icon and/or text, never color alone. */
