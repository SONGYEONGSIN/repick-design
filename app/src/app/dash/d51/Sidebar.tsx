...
import { ACCENT_SUBTLE, ACCENT_TEXT, BORDER, DISPLAY_MONO, FOCUS, HOVER_BG, PANEL_BG, SURFACE_INSET, TEXT_AUX, TEXT_MUTED, TEXT_PRIMARY, TRANSITION, cx } from "./tokens";
        ...
        <div role="listbox" aria-label="Select workspace" className={cx("absolute left-3 right-3 top-full z-40 mt-1.5 rounded-xl border p-1", BORDER, PANEL_BG, "shadow-xl shadow-black/40")}>
          {WORKSPACES.map((w) => {
            ...
            return (
              ...
              >
                <span className="min-w-0 flex-1">
                  <span className={cx("block truncate text-sm font-medium", selected ? "text-rose-300" : TEXT_PRIMARY)}>{w.name}</span>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-11 shrink-0 items-center gap-2 px-4">
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-[9px] bg-rose-600 text-white">
          <BRAND.Icon size={16} aria-hidden="true" strokeWidth={2.25} />
        </span>
        <span className={cx("text-[15px] font-semibold leading-none tracking-tight", TEXT_PRIMARY)} style={DISPLAY_MONO}>
          {BRAND.name}
        </span>
      </div>
    ...
    <>
      ...
      {mobileOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className={cx("absolute inset-y-0 left-0 w-72 max-w-[85vw] border-r shadow-2xl shadow-black/50", BORDER, PANEL_BG)}>
