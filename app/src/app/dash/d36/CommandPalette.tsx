...
import { SEGMENTS, STAGES, type StageId } from "./data";
import { BORDER, FOCUS_RING, TEXT_CAPTION, TEXT_PRIMARY, cx } from "./tokens";

type ResultItem =
  | { kind: "stage"; id: StageId; label: string; hint: string; Icon: (typeof STAGES)[number]["Icon"] }
  | { kind: "segment"; id: string; label: string; hint: string; Icon: (typeof SEGMENTS)[number]["Icon"] };
...
export default function CommandPalette({
  ...
  const allItems: ResultItem[] = useMemo(
    () => [
      ...STAGES.map((s) => ({ kind: "stage" as const, id: s.id, label: s.label, hint: "Funnel stage", Icon: s.Icon })),
      ...SEGMENTS.map((s) => ({ kind: "segment" as const, id: s.id, label: s.label, hint: "Traffic segment", Icon: s.Icon })),
    ],
    [],
  );
  ...
  function choose(idx: number) {
    ...
    if (!it) return;
    if (it.kind === "stage") onSelectStage(it.id);
    else onSelectSegment(it.id);
    onClose();
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-[12vh]">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Search stages and segments"
        onKeyDown={onKeyDown}
      ...
      >
        <div className={cx("flex items-center gap-2 border-b px-3.5", BORDER)}>
          ...
          <input
            ...
            placeholder="Search stages, segments, reasons"
              ...
              <li key={`${it.kind}-${it.id}`}>
                <button
                  ...
                  className={cx(
                    ...
                    i === active ? "bg-violet-50 dark:bg-violet-500/10" : "hover:bg-zinc-50 dark:hover:bg-zinc-800",
                  )}
                >
        ...
        <div className={cx("flex items-center justify-between border-t px-3.5 py-2 text-[11px]", BORDER, TEXT_CAPTION)}>
          <span>Select a stage or segment to sync the funnel and detail panel</span>
