...
import { Check, ChevronsUpDown, SlidersHorizontal, Sun, X } from "lucide-react";
...
function WorkspaceSwitcher() {
  ...
  return (
    <div ref={ref} className="relative px-3 pt-3">
      ...
      {open ? (
        <div role="listbox" aria-label="Select workspace" className={cx("absolute left-3 right-3 top-full z-40 mt-1.5 overflow-hidden rounded-xl border p-1", BORDER, "bg-white shadow-lg shadow-zinc-950/10")}>
          {WORKSPACES.map((w) => {
    ...
    <div className="flex h-full flex-col">
      <div className="flex h-11 shrink-0 items-center gap-2 px-4">
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-[8px] bg-teal-600 text-white">
          <Sun size={16} aria-hidden="true" strokeWidth={2.25} />
        </span>
        <span
          className={cx("text-[15px] font-semibold leading-none tracking-tight", TEXT_PRIMARY)}
                  ...
                  <li key={item.id}>
                    <a
                      ...
                      className={cx(
                        ...
                        TRANSITION,
                        FOCUS_VISIBLE,
                        item.active ? cx(ACCENT_SUBTLE, "font-semibold") : cx(TEXT_CAPTION, "hover:bg-zinc-100 hover:text-zinc-900"),
                      )}
                    >
                      <item.Icon size={17} aria-hidden="true" />
                      {item.label}
    ...
    <>
      ...
      {mobileOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className={cx("absolute inset-y-0 left-0 w-72 max-w-[85vw] border-r shadow-lg shadow-zinc-950/10", BORDER, "bg-white")}>
