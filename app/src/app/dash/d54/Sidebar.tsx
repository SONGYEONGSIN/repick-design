...
import { ACCENT_SUBTLE, BORDER, FOCUS, HOVER_BG, PANEL_BG, SURFACE_INSET, TEXT_AUX, TEXT_MUTED, TEXT_PRIMARY, TRANSITION, cx } from "./tokens";
        ...
        <div role="listbox" aria-label="Select workspace" className={cx("absolute left-3 right-3 top-full z-40 mt-1.5 rounded-xl border p-1", BORDER, PANEL_BG, "shadow-xl shadow-black/40")}>
          {WORKSPACES.map((w) => {
            ...
            return (
              ...
              >
                <span className="min-w-0 flex-1">
                  <span className={cx("block truncate text-sm font-medium", selected ? "text-blue-300" : TEXT_PRIMARY)}>{w.name}</span>
                  <span className={cx("block truncate text-[11px] font-normal", TEXT_AUX)}>{w.plan}</span>
                </span>
                {selected ? <Check size={15} aria-hidden="true" className="shrink-0 text-blue-400" /> : null}
              </button>
            );
          })}
        </div>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-11 shrink-0 items-center gap-2 px-4">
        ...
        <span className={cx("text-[15px] font-semibold leading-none tracking-tight", TEXT_PRIMARY)} style={{ fontFamily: "var(--font-display-grotesk)" }}>
          {BRAND.name}
        </span>
      </div>
                  ...
                  <li key={item.id}>
                    <a
                      ...
                      className={cx(
                        ...
                        TRANSITION,
                        FOCUS,
                        item.active ? cx(ACCENT_SUBTLE, "font-semibold") : cx("font-medium", TEXT_AUX, "hover:bg-white/[0.06] hover:text-zinc-100"),
                      )}
                    >
                      <item.Icon size={17} aria-hidden="true" />
                      {item.label}
