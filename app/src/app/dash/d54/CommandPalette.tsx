...
export default function CommandPalette({ onClose, onSelectRoute }: { onClose: () => void; onSelectRoute: (id: string) => void }) {
      ...
      <div role="dialog" aria-modal="true" aria-label="Command palette" onClick={(e) => e.stopPropagation()} className={cx("w-full max-w-xl rounded-2xl border shadow-2xl shadow-black/50", BORDER, PANEL_BG)}>
        <div className={cx("flex items-center gap-2.5 border-b px-3 py-2", BORDER)}>
          ...
          <input
            ...
            type="text"
            placeholder="Search routes, drivers, zones…"
            aria-label="Search routes, drivers, zones"
        ...
        <div className="max-h-[60vh] overflow-y-auto p-2 [scrollbar-width:thin]">
          {results.length === 0 ? <p className={cx("px-2.5 py-6 text-center text-sm font-normal", TEXT_AUX)}>Nothing matches that.</p> : null}
          {results.length > 0 ? (
            <div>
              ...
              {results.map((e) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    if (e.kind === "route") onSelectRoute(e.id);
                    onClose();
                  }}
