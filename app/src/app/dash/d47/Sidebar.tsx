...
import { Check, ChevronsUpDown, FlaskConical, SlidersHorizontal, X } from "lucide-react";
import Image from "next/image";
import { useState } from "react";
import { BRAND, CURRENT_USER, NAV_SECTIONS, unsplashAvatar, WORKSPACES } from "./data";
import { ACCENT_SUBTLE, ACCENT_TEXT, BORDER, FOCUS_VISIBLE, FOCUS_VISIBLE_INSET, HOVER_ACTIVE_BG, TEXT_CAPTION, TEXT_PRIMARY, TRANSITION, cx } from "./tokens";
  ...
  return (
    <div ref={ref} className="relative px-3 pt-3">
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className={cx("flex h-11 w-full items-center gap-2 rounded-xl border px-2.5 text-left", BORDER, "bg-zinc-900", HOVER_ACTIVE_BG, TRANSITION, FOCUS_VISIBLE)}
      >
      ...
      {open ? (
        <div role="listbox" aria-label="Select workspace" className={cx("absolute left-3 right-3 top-full z-40 mt-1.5 overflow-hidden rounded-xl border p-1", BORDER, "bg-zinc-900 shadow-lg shadow-black/30")}>
          {WORKSPACES.map((w) => {
    ...
    <div className="flex h-full flex-col">
      <div className="flex h-11 shrink-0 items-center gap-2 px-4">
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-[8px] bg-cyan-400 text-zinc-950">
          <FlaskConical size={16} aria-hidden="true" strokeWidth={2.25} />
        </span>
                  ...
                  return (
                    <li key={item.id}>
                      <span aria-disabled="true" className={cx("flex min-h-11 cursor-not-allowed items-center gap-3 rounded-lg px-2.5 py-2 text-sm font-medium", TEXT_CAPTION)}>
                        ...
                        {item.label}
                        <span className="ml-auto rounded-full bg-zinc-800 px-1.5 py-0.5 text-[10px] font-medium text-zinc-300">Soon</span>
                      </span>
                    </li>
                  );
                }
