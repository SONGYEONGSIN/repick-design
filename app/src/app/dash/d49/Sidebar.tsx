...
function WorkspaceSwitcher() {
  ...
  return (
    <div ref={ref} className="relative px-3 pt-3">
      ...
      {open ? (
        <div role="listbox" aria-label="Select workspace" className={cx("absolute left-3 right-3 top-full z-40 mt-1.5 rounded-xl border p-1", BORDER, "bg-zinc-900 shadow-xl shadow-black/60")}>
          {WORKSPACES.map((w) => {
            ...
            return (
              ...
              >
                <span className="min-w-0 flex-1">
                  <span className={cx("block truncate text-sm font-medium", selected ? "text-lime-300" : TEXT_PRIMARY)}>{w.name}</span>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-11 shrink-0 items-center gap-2 px-4">
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-[9px] bg-lime-400 text-zinc-950">
                  ...
                  <li key={item.id}>
                    <a
                      ...
                      className={cx(
                        ...
                        TRANSITION,
                        FOCUS,
                        item.active ? cx(ACCENT_SUBTLE, "font-semibold") : cx("font-medium", TEXT_AUX, "hover:bg-white/5 hover:text-zinc-50"),
                      )}
                    >
                      <item.Icon size={17} aria-hidden="true" />
                      {item.label}
      ...
      <div className={cx("border-t p-3", BORDER)}>
        <div className="flex items-center gap-2.5 rounded-lg px-2 py-2">
          <Image
            ...
            width={28}
            height={28}
            className="h-7 w-7 shrink-0 rounded-full border border-white/10 bg-zinc-800 object-cover"
          />
    ...
    <>
      ...
      {mobileOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          <button type="button" aria-label="Close navigation" onClick={onCloseMobile} className="absolute inset-0 bg-black/70" />
          <aside className={cx("absolute inset-y-0 left-0 w-72 max-w-[85vw] border-r shadow-2xl shadow-black/80", BORDER, PANEL_BG)}>
