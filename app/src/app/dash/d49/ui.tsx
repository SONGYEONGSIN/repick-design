...
/**
 * The ± marker. Direction is carried by the ARROW plus the signed figure — the tint is only ever
 * the third, redundant channel. Strip colour and this component still reads correctly.
 */
export function DirectionMark({
  ...
  return (
    <span className={cx("inline-flex shrink-0 items-center", className)}>
      <Icon size={size} strokeWidth={2.5} aria-hidden="true" className={down ? "text-lime-400" : "text-zinc-300"} />
        ...
        return (
          <button
            ...
            className={cx(
              ...
              TRANSITION,
              FOCUS,
              active ? "bg-zinc-100 font-semibold text-zinc-950" : cx("font-medium", TEXT_AUX, "hover:bg-white/5 hover:text-zinc-50"),
            )}
          >
            {opt.label}
          </button>
 ...
 * fixed viewBox, so `preserveAspectRatio` never has to stretch a label or squash a marker — the
 * waterfall keeps its bar geometry honest at 1280px and at 1920px alike.
 */
...
/** All generated coordinates go through this — determinism gate wants 2 decimals, max. */
