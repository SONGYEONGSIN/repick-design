...
export default function CommandPalette({ onClose, onSelectCase }: { onClose: () => void; onSelectCase: (id: string) => void }) {
      ...
      <div role="dialog" aria-modal="true" aria-label="Command palette" onClick={(e) => e.stopPropagation()} className={cx("w-full max-w-xl rounded-2xl border shadow-2xl shadow-zinc-900/20", BORDER, PANEL_BG)}>
        <div className={cx("flex items-center gap-2.5 border-b px-3 py-2", BORDER)}>
          ...
          <input
            ...
            type="text"
            placeholder="Search cases by vendor or category…"
            aria-label="Search cases by vendor or category"
        ...
        <div className="max-h-[60vh] overflow-y-auto p-2 [scrollbar-width:thin]">
          {results.length === 0 ? <p className={cx("px-2.5 py-6 text-center text-sm font-normal", TEXT_AUX)}>No cases match that.</p> : null}
          {results.length > 0 ? (
            <div>
