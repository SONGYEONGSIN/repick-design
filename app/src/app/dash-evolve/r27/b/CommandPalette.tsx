...
export default function CommandPalette({ onClose, onSelectNode }: { onClose: () => void; onSelectNode: (id: string) => void }) {
  ...
  const results = useMemo(() => SEARCH_ENTRIES.filter((e) => q === "" || e.title.toLowerCase().includes(q) || e.meta.toLowerCase().includes(q)).slice(0, 8), [q]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-zinc-900/40 px-4 pt-20 sm:pt-24" role="presentation" onClick={onClose}>
      <div role="dialog" aria-modal="true" aria-label="Search the cost tree" onClick={(e) => e.stopPropagation()} className={cx("w-full max-w-xl rounded-2xl border shadow-2xl shadow-zinc-900/20", BORDER, PANEL_BG)}>
        <div className={cx("flex items-center gap-2.5 border-b px-3 py-2", BORDER)}>
          ...
          <input
            ...
            type="text"
            placeholder="Search by region, service, resource type or SKU&hellip;"
            aria-label="Search by region, service, resource type or SKU"
        ...
        <div className="max-h-[60vh] overflow-y-auto p-2 [scrollbar-width:thin]">
          {results.length === 0 ? <p className={cx("px-2.5 py-6 text-center text-sm font-normal", TEXT_AUX)}>Nothing in the tree matches that.</p> : null}
          {results.length > 0 ? (
            <div>
