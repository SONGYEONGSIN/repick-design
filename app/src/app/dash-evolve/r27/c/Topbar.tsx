...
import { Bell, HelpCircle, LogOut, Menu, Search, Settings, UploadCloud } from "lucide-react";
...
const NOTIFICATIONS = [
  { id: "n1", text: "Anchorline Supply crossed into the At-risk band", time: "18m ago" },
  { id: "n2", text: "Kestrel Pack Co. contract renews in 41 days", time: "2h ago" },
  { id: "n3", text: "Quarterly scorecard digest is ready", time: "5h ago" },
];

/**
 * Every control here is individually 44px tall (h-11 / w-11), not just the header bar itself —
 * a 44px header with a 36px button inside it still fails the target-size rule this page is held to.
 */
export default function Topbar({ onOpenPalette, onOpenMobileNav }: { onOpenPalette: () => void; onOpenMobileNav: () => void }) {
  ...
  return (
    <header className={cx("sticky top-0 z-30 flex h-16 shrink-0 items-center gap-2 border-b px-3 sm:px-4", BORDER, PANEL_BG)}>
      ...
      >
        ...
        <span className="sr-only truncate sm:not-sr-only sm:inline">Search vendors</span>
      ...
      <div className="ml-auto flex items-center gap-1.5">
        <button type="button" className={cx("hidden h-11 items-center gap-1.5 rounded-lg px-3.5 text-sm font-semibold sm:inline-flex", ACCENT_SOLID, TRANSITION, FOCUS)}>
          <UploadCloud size={15} aria-hidden="true" strokeWidth={2.25} />
          Export comparison
        </button>
        ...
        <div ref={notifRef} className="relative">
          ...
          {notifOpen ? (
            <div role="menu" aria-label="Notifications" className={cx("absolute right-0 top-full z-40 mt-2 w-80 max-w-[calc(100vw-1.5rem)] rounded-xl border", BORDER, PANEL_BG, "shadow-xl shadow-black/40")}>
        ...
        <div ref={userRef} className="relative">
          ...
          {userOpen ? (
            <div role="menu" aria-label="Account" className={cx("absolute right-0 top-full z-40 mt-2 w-60 rounded-xl border p-1", BORDER, PANEL_BG, "shadow-xl shadow-black/40")}>
              ...
              <div className="p-1">
                {[
                  ...
                  { label: "Scoring methodology", Icon: HelpCircle },
                  { label: "Log out", Icon: LogOut },
                ].map((item) => (
                  <button key={item.label} type="button" role="menuitem" onClick={() => setUserOpen(false)} className={cx("flex min-h-11 w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left text-sm font-medium", TEXT_PRIMARY, HOVER_BG, TRANSITION, FOCUS)}>
