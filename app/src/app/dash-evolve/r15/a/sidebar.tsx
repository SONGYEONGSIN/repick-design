...
import { Check, ChevronsUpDown, SlidersHorizontal, Waypoints, X } from "lucide-react";
import { useState } from "react";
import { BRAND, CURRENT_USER, NAV_SECTIONS, WORKSPACES } from "./data";
import { ACCENT_SUBTLE, ACCENT_TEXT, BORDER, DISPLAY_FONT, FOCUS_RING, HOVER_ACTIVE_BG, TEXT_CAPTION, TEXT_PRIMARY, TRANSITION, cx } from "./tokens";
import { Avatar, EyebrowLabel, useOutsideClose } from "./ui";
  ...
  return (
    <div ref={ref} className="relative px-3 pt-3">
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className={cx("flex h-11 w-full items-center gap-2 rounded-xl border px-2.5 text-left", BORDER, "bg-zinc-900/60", HOVER_ACTIVE_BG, TRANSITION, FOCUS_RING)}
      >
            ...
            return (
              <button
                ...
                className={cx(
                  ...
                  TRANSITION,
                  FOCUS_RING,
                  selected ? ACCENT_SUBTLE : HOVER_ACTIVE_BG,
                )}
              >
                <span className="min-w-0 flex-1">
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-11 shrink-0 items-center gap-2 px-4">
        <span className={cx("grid h-7 w-7 shrink-0 place-items-center rounded-[8px] text-white", "bg-sky-700")}>
                  ...
                  return (
                    <li key={item.id}>
                      <span aria-disabled="true" className={cx("flex min-h-11 cursor-not-allowed items-center gap-3 rounded-lg px-2.5 py-2 text-sm font-medium", TEXT_CAPTION)}>
                        ...
                        {item.label}
                        <span className={cx("ml-auto rounded-full px-1.5 py-0.5 text-[10px] font-medium", "bg-zinc-800", TEXT_CAPTION)}>Soon</span>
                      </span>
                    </li>
                  );
                }
                  ...
                  <li key={item.id}>
                    <a
                      ...
                      className={cx(
                        ...
                        TRANSITION,
                        FOCUS_RING,
                        item.active ? cx(ACCENT_SUBTLE, "font-semibold") : cx(TEXT_CAPTION, "hover:bg-white/5 hover:text-zinc-50"),
                      )}
                    >
                      <item.Icon size={17} aria-hidden="true" />
                      {item.label}
      ...
      <div className={cx("border-t p-3", BORDER)}>
        <div className="flex items-center gap-2.5 rounded-lg px-2 py-2">
          <Avatar avatarId={CURRENT_USER.avatarId} name={CURRENT_USER.name} size={28} />
