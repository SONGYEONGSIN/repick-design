...
const NOTIFICATIONS = [
  { id: "n1", text: "Checkout CTA copy crossed 95% confidence — lift is now statistically significant.", time: "1h ago" },
  { id: "n2", text: "Pricing page hero rewrite is trending negative and significant — consider pausing.", time: "5h ago" },
  { id: "n3", text: "Referral incentive amount finished its run — final report is ready.", time: "1d ago" },
];
  ...
  return (
    <header className={cx("flex h-11 shrink-0 items-center gap-2 border-b px-3 sm:px-4", BORDER, "bg-zinc-950")}>
      ...
      <button
        ...
        className={cx("ml-1 flex h-11 flex-1 items-center gap-2 rounded-lg border pl-3 pr-2.5 text-left text-sm sm:max-w-sm", BORDER, "bg-zinc-900 hover:bg-zinc-800", TEXT_CAPTION, TRANSITION, FOCUS_VISIBLE)}
      >
        <Search size={16} aria-hidden="true" className="shrink-0" />
        <span className="sr-only truncate sm:not-sr-only sm:inline">Search experiments, owners, metrics</span>
      ...
      <div className="ml-auto flex items-center gap-1.5">
        ...
        <div ref={notifRef} className="relative">
          <button
            ...
            className={cx("relative grid h-11 w-11 place-items-center rounded-full border", BORDER, "bg-zinc-900", HOVER_ACTIVE_BG, TRANSITION, FOCUS_VISIBLE)}
          >
          ...
          {notifOpen ? (
            <div role="menu" aria-label="Notifications" className={cx("absolute right-0 top-full z-40 mt-2 w-80 max-w-[calc(100vw-1.5rem)] overflow-hidden rounded-xl border", BORDER, "bg-zinc-900 shadow-lg shadow-black/30")}>
        ...
        <div ref={userRef} className="relative">
          <button
            ...
            className={cx("flex h-11 items-center gap-2 rounded-full border pl-1 pr-2", BORDER, "bg-zinc-900", HOVER_ACTIVE_BG, TRANSITION, FOCUS_VISIBLE)}
          >
          ...
          {userOpen ? (
            <div role="menu" aria-label="Account" className={cx("absolute right-0 top-full z-40 mt-2 w-60 overflow-hidden rounded-xl border p-1", BORDER, "bg-zinc-900 shadow-lg shadow-black/30")}>
              ...
              <div className="p-1">
                {[
                  { label: "Experiment settings", Icon: Settings },
