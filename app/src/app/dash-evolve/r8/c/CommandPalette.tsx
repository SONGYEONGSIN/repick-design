...
import { ancestorsOf, NODES, STATUS_META } from "./data";
...
export default function CommandPalette({ onClose, onSelectNode }: { onClose: () => void; onSelectNode: (id: string) => void }) {
  ...
  const results = useMemo(() => {
    ...
    const pool = q
      ? NODES.filter((n) => n.name.toLowerCase().includes(q) || n.leadName.toLowerCase().includes(q) || n.leadTitle.toLowerCase().includes(q))
      : NODES;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-[12vh]">
      ...
      <div role="dialog" aria-modal="true" aria-label="팀/리드 검색" onKeyDown={onKeyDown} className={cx("relative w-full max-w-lg overflow-hidden rounded-2xl border shadow-xl", BORDER, "bg-white dark:bg-zinc-900")}>
        ...
        <ul className="max-h-[46vh] overflow-y-auto p-1.5 [scrollbar-width:thin]">
          {results.length === 0 ? (
            ...
            results.map((n, i) => {
              ...
              const trail = ancestorsOf(n.id);
              const crumb = trail.length ? trail[trail.length - 1].name : "최상위";
              return (
                <li key={n.id}>
                  <button
                    type="button"
                    ...
                    className={cx("flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left", i === active ? "bg-teal-50 dark:bg-teal-500/10" : "hover:bg-zinc-50 dark:hover:bg-zinc-800")}
                  >
                    ...
                    <span className={cx("shrink-0 text-xs font-semibold", NUM, TEXT_CAPTION)}>{n.utilization}%</span>
        ...
        <div className={cx("flex items-center justify-between border-t px-3.5 py-2 text-[11px]", BORDER, TEXT_CAPTION)}>
          <span>팀을 선택하면 조직도·상세 패널·로스터가 동기화됩니다</span>
