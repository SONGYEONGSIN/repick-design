...
import { Bell, HelpCircle, LogOut, Menu, Search, Settings, UserPlus2 } from "lucide-react";
...
const NOTIFICATIONS = [
  { id: "n1", text: "Infrastructure & SRE 가동률이 111%로 과부하 구간에 진입했습니다.", time: "12분 전" },
  { id: "n2", text: "Recruiting 팀에 신규 채용 요청 2건이 추가되었습니다.", time: "48분 전" },
  { id: "n3", text: "Web App 팀 가동률이 지난 분기 대비 9%p 상승했습니다.", time: "3시간 전" },
];
  ...
  return (
    <header className={cx("flex h-11 shrink-0 items-center gap-2 border-b px-3 sm:px-4", BORDER, "bg-white dark:bg-zinc-950")}>
      ...
      >
        ...
        <span className="hidden truncate sm:inline">팀, 리드, 부문 검색</span>
      ...
      <div className="ml-auto flex items-center gap-1.5">
        <button
          type="button"
          onClick={onOpenPalette}
          className={cx("hidden h-11 items-center gap-1.5 rounded-lg px-3.5 text-sm font-medium sm:inline-flex", "bg-teal-600 text-white hover:bg-teal-500 active:bg-teal-700", TRANSITION, FOCUS_RING)}
        >
          <UserPlus2 size={15} aria-hidden="true" />
          팀 추가
        </button>
