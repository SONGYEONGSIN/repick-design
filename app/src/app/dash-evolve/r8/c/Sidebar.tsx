...
function WorkspaceSwitcher() {
  ...
  return (
    <div ref={ref} className="relative px-3 pt-3">
      ...
      >
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-md bg-teal-50 text-[11px] font-semibold text-teal-700 dark:bg-teal-500/10 dark:text-teal-300">
          {ws.name.slice(0, 1)}
        </span>
            ...
            return (
              <button
                ...
                className={cx(
                  ...
                  TRANSITION,
                  FOCUS_RING,
                  selected ? "bg-teal-50 dark:bg-teal-500/10" : HOVER_ACTIVE_BG,
                )}
              >
                <span className="min-w-0 flex-1">
                ...
                {selected && <Check size={16} aria-hidden="true" className="shrink-0 text-teal-600 dark:text-teal-400" />}
              </button>
            );
          })}
        </div>
                ...
                return (
                  <li key={item.id}>
                    ...
                    >
                      ...
                      {item.label}
                      {item.badge ? (
                        <span className="ml-auto grid h-5 min-w-5 place-items-center rounded-full bg-teal-600 px-1 text-[11px] font-semibold text-white">{item.badge}</span>
                      ) : null}
                    </a>
                  </li>
                );
