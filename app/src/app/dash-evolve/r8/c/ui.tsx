...
import { round2, STATUS_META, type StatusId } from "./data";
import { BORDER, FOCUS_RING, NUM, TEXT_CAPTION, TEXT_PRIMARY, TRANSITION, cx, type Tone, TONE } from "./tokens";
...
/** 상태 배지(색 + 텍스트 병행) — healthy / at-risk / overloaded. */
export function StatusBadge({ status }: { status: StatusId }) {
  const m = STATUS_META[status];
  return (
    <span className={cx("inline-flex items-center gap-1.5 whitespace-nowrap rounded-full border px-2 py-0.5 text-xs font-medium leading-none", m.text, m.bg, m.border)}>
      <span aria-hidden="true" className={cx("h-1.5 w-1.5 shrink-0 rounded-full", m.dot)} />
      {m.label}
    </span>
  );
}
...
/** 수평 진행률 바 — 캐파시티/가동률 시각화 공용. value/max 0 이상, 색+수치 병행. */
...
/** 결정론 가동률 추이 스파크라인 — 라인 + 은은한 면적 + 종점. 좌표 소수 2자리. */
