...
import { Bell, HelpCircle, LogOut, Menu, MessagesSquare, Search, Settings } from "lucide-react";
import Image from "next/image";
import { useEffect, useRef, useState } from "react";
import { COPILOT_NAME, CURRENT_USER, unsplashAvatar } from "./data";
import { BORDER, FOCUS_RING, HOVER_ACTIVE_BG, TEXT_CAPTION, TEXT_PRIMARY, TRANSITION, cx } from "./tokens";

const NOTIFICATIONS = [
  { id: "n1", text: "Northstar Retail 헬스가 46점 아래로 떨어졌습니다.", time: "4분 전" },
  { id: "n2", text: "EMEA 리전 MRR이 전주 대비 2.6% 늘었습니다.", time: "26분 전" },
  { id: "n3", text: "Cinderline의 결제가 2회 연속 실패했습니다.", time: "3시간 전" },
];
  ...
  return (
    <header className={cx("flex h-11 shrink-0 items-center gap-2 border-b px-3 sm:px-4", BORDER, "bg-white dark:bg-zinc-950")}>
      ...
      >
        ...
        <span className="hidden truncate sm:inline">계정, 리전, 지표 검색</span>
        <span
          aria-hidden="true"
      ...
      <div className="ml-auto flex items-center gap-1.5">
        <button
          ...
          aria-label={`${COPILOT_NAME} 코파일럿 ${chatVisible ? "닫기" : "열기"}`}
          className={cx(
        ...
        >
          ...
          <span className="hidden sm:inline">{COPILOT_NAME}에게 묻기</span>
        </button>
