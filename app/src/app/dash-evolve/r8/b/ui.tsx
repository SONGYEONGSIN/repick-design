...
import { round2, STATUS_META, type AccountStatus } from "./data";
import {
  BORDER,
  CARD,
  FOCUS_RING,
...
/** 계정 상태 배지 — 색 + 텍스트 병행(색만으로 의미 전달 금지). */
export function StatusBadge({ status }: { status: AccountStatus }) {
  ...
  return (
    ...
    >
      <span aria-hidden="true" className={cx("h-1.5 w-1.5 shrink-0 rounded-full", TONE[m.tone].dot)} />
      {m.label}
    </span>
  );
}
...
/** 결정론 히스토리 스파크라인 — 라인 + 은은한 면적 + 종점. 좌표 소수 2자리. */
export function Sparkline({ values, stroke, fill, className }: { values: number[]; stroke: string; fill: string; className?: string }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className={cx("h-full w-full", className)} role="img" aria-hidden="true" focusable="false" preserveAspectRatio="none">
      ...
      <circle cx={last.x} cy={last.y} r={2.4} className={cx(stroke, fill)} strokeWidth={1.5} />
    </svg>
  );
}
