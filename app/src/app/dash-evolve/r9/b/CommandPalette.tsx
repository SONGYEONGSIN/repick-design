...
import { formatVolume, NODES, RELIABILITY_META } from "./data";
...
export default function CommandPalette({ onClose, onSelectNode }: { onClose: () => void; onSelectNode: (id: string) => void }) {
  ...
  const results = useMemo(() => {
    ...
    const pool = q
      ? NODES.filter((n) => n.name.toLowerCase().includes(q) || n.owner.toLowerCase().includes(q) || n.shortLabel.toLowerCase().includes(q))
      : NODES;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-[12vh]">
      ...
      <div role="dialog" aria-modal="true" aria-label="서비스 검색" onKeyDown={onKeyDown} className={cx("relative w-full max-w-lg overflow-hidden rounded-2xl border shadow-xl", BORDER, "bg-white dark:bg-zinc-900")}>
        ...
        <ul className="max-h-[46vh] overflow-y-auto p-1.5 [scrollbar-width:thin]">
          {results.length === 0 ? (
            ...
            results.map((n, i) => {
              const m = RELIABILITY_META[n.reliability];
              return (
                <li key={n.id}>
                  <button
                    type="button"
                  ...
                  >
                    <span aria-hidden="true" className={cx("h-2 w-2 shrink-0 rounded-full", m.dot)} />
                    <span className="min-w-0 flex-1">
                      ...
                      <span className={cx("block truncate text-xs", TEXT_CAPTION)}>
                        {n.owner} · {formatVolume(n.requestVolume)}
                      </span>
                    </span>
                    <span className={cx("shrink-0 text-xs font-semibold", NUM, TEXT_CAPTION)}>{n.errorRate.toFixed(1)}%</span>
        ...
        <div className={cx("flex items-center justify-between border-t px-3.5 py-2 text-[11px]", BORDER, TEXT_CAPTION)}>
          <span>서비스를 선택하면 그래프·상세 패널·테이블이 동기화됩니다</span>
