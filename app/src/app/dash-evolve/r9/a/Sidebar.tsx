...
function WorkspaceSwitcher() {
  ...
  return (
    <div ref={ref} className="relative px-3 pt-3">
      ...
      >
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-md bg-sky-50 text-[11px] font-semibold text-sky-700 dark:bg-sky-500/10 dark:text-sky-300">
          {ws.name.slice(0, 1)}
        </span>
            ...
            return (
              <button
                ...
                className={cx(
                  ...
                  TRANSITION,
                  FOCUS_RING,
                  selected ? "bg-sky-50 dark:bg-sky-500/10" : HOVER_ACTIVE_BG,
                )}
              >
                <span className="min-w-0 flex-1">
                ...
                {selected && <Check size={16} aria-hidden="true" className="shrink-0 text-sky-600 dark:text-sky-400" />}
              </button>
            );
          })}
        </div>
                ...
                return (
                  <li key={item.id}>
                    ...
                    >
                      ...
                      {item.label}
                      {item.badge ? (
                        <span className="ml-auto grid h-5 min-w-5 place-items-center rounded-full bg-rose-600 px-1 text-[11px] font-semibold text-white">
                          {item.badge}
                        </span>
                      ) : null}
                    </a>
