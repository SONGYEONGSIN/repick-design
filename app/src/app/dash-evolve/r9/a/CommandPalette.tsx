...
import { INCIDENTS, SERVICES, type ServiceId } from "./data";
...
export default function CommandPalette({
  ...
  const allItems: ResultItem[] = useMemo(
    () => [
      ...SERVICES.map((s) => ({ kind: "service" as const, id: s.id, label: s.label, hint: `Service · SLO ${s.sloTargetPct}%` })),
  ...
  function choose(idx: number) {
    ...
    if (!it) return;
    if (it.kind === "service") onSelectService(it.id);
    else onSelectIncident(it.id);
    onClose();
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-[12vh]">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Search services and incidents"
        onKeyDown={onKeyDown}
      ...
      >
          ...
          {results.length === 0 ? (
            ...
            results.map((it, i) => (
              <li key={`${it.kind}-${it.id}`}>
                ...
                >
                  <span className="grid h-7 w-7 shrink-0 place-items-center rounded-md bg-zinc-100 text-[10px] font-semibold uppercase dark:bg-zinc-800">
                    {it.kind === "service" ? "SVC" : "INC"}
                  </span>
        ...
        <div className={cx("flex items-center justify-between border-t px-3.5 py-2 text-[11px]", BORDER, TEXT_CAPTION)}>
          <span>Select a service or incident to sync the gauge cluster and detail panel</span>
