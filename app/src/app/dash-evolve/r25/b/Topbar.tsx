...
import { ACCENT_SOLID, BORDER, FOCUS, HOVER_BG, PANEL_BG, SURFACE_INSET, TEXT_MUTED, TEXT_PRIMARY, TRANSITION, cx } from "./tokens";
...
export default function Topbar({ onOpenPalette, onOpenMobileNav }: { onOpenPalette: () => void; onOpenMobileNav: () => void }) {
  ...
  return (
    <header className={cx("sticky top-0 z-30 flex h-11 shrink-0 items-center gap-2 border-b px-3 sm:px-4", BORDER, PANEL_BG)}>
      <button type="button" onClick={onOpenMobileNav} className={cx("grid h-11 w-11 shrink-0 place-items-center rounded-lg text-sm font-medium lg:hidden", TEXT_MUTED, HOVER_BG, TRANSITION, FOCUS)}>
      ...
      >
        ...
        <span className="sr-only truncate sm:not-sr-only sm:inline">Search runs, sellers, sections</span>
        <span aria-hidden="true" className={cx("ml-auto hidden shrink-0 rounded-md border px-1.5 py-0.5 text-[11px] font-medium sm:inline-flex", BORDER, PANEL_BG, TEXT_MUTED)}>
          &#8984;K
        </span>
      </button>
      ...
      <div className="ml-auto flex items-center gap-1.5">
        ...
        <div ref={notifRef} className="relative">
          ...
          >
            <Bell size={17} aria-hidden="true" className={TEXT_MUTED} />
            <span className="absolute right-2.5 top-2.5 h-1.5 w-1.5 rounded-full bg-orange-600" aria-hidden="true" />
              ...
              <ul className="divide-y divide-zinc-100">
                {NOTIFICATIONS.map((n) => (
                  <li key={n.id} className="px-4 py-2.5">
                    ...
                    <p className={cx("mt-0.5 text-[11px] font-normal", TEXT_MUTED)}>{n.time}</p>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div ref={userRef} className="relative">
          ...
          >
            <Image
              ...
              width={30}
              height={30}
              className="h-[30px] w-[30px] shrink-0 rounded-full bg-zinc-200 object-cover"
            />
          ...
          {userOpen ? (
            <div role="menu" aria-label="Account" className={cx("absolute right-0 top-full z-40 mt-2 w-60 rounded-xl border p-1", BORDER, PANEL_BG, "shadow-xl shadow-zinc-900/10")}>
              <div className={cx("border-b px-3 py-2.5", BORDER)}>
                ...
                <p className={cx("truncate text-[11px] font-normal", TEXT_MUTED)}>{CURRENT_USER.email}</p>
              </div>
              <div className="p-1">
                {[
                  { label: "Console settings", Icon: Settings },
                  { label: "Payments docs", Icon: HelpCircle },
                  ...
                  <button key={item.label} type="button" role="menuitem" onClick={() => setUserOpen(false)} className={cx("flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left text-sm font-medium", TEXT_PRIMARY, HOVER_BG, TRANSITION, FOCUS)}>
                    <item.Icon size={16} aria-hidden="true" className={TEXT_MUTED} />
                    {item.label}
                  </button>
                ))}
              </div>
