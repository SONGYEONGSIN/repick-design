...
import { ACCENT_SOLID, ACCENT_SUBTLE, ACCENT_TEXT, BORDER, FOCUS, HOVER_BG, PANEL_BG, SURFACE_INSET, TEXT_MUTED, TEXT_PRIMARY, TRANSITION, cx } from "./tokens";
  ...
  return (
    <div ref={ref} className="relative px-3 pt-3">
      ...
      >
        ...
        <ChevronsUpDown size={15} aria-hidden="true" className={TEXT_MUTED} />
      </button>

      {open ? (
        <div role="listbox" aria-label="Select workspace" className={cx("absolute left-3 right-3 top-full z-40 mt-1.5 rounded-xl border p-1", BORDER, PANEL_BG, "shadow-xl shadow-zinc-900/10")}>
          {WORKSPACES.map((w) => {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-11 shrink-0 items-center gap-2 px-4">
        <span className={cx("grid h-7 w-7 shrink-0 place-items-center rounded-[9px]", ACCENT_SOLID)}>
            ...
            <ul className="flex flex-col gap-0.5">
              {section.items.map((item) => {
                if (item.disabled) {
                  return (
                    <li key={item.id}>
                      <span aria-disabled="true" className={cx("flex min-h-11 cursor-not-allowed items-center gap-3 rounded-lg px-2.5 py-2 text-sm font-medium", TEXT_MUTED)}>
                  ...
                  <li key={item.id}>
                    <a
                      ...
                      className={cx(
                        ...
                        TRANSITION,
                        FOCUS,
                        item.active ? cx(ACCENT_SUBTLE, "font-semibold") : cx("font-medium", TEXT_MUTED, "hover:bg-zinc-100 hover:text-zinc-900"),
                      )}
                    >
                      <item.Icon size={17} aria-hidden="true" />
                      {item.label}
      ...
      <div className={cx("border-t p-3", BORDER)}>
        <div className="flex items-center gap-2.5 rounded-lg px-2 py-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className={cx("truncate text-[11px] font-normal", TEXT_MUTED)}>{CURRENT_USER.role}</p>
          </div>
          <SlidersHorizontal size={15} aria-hidden="true" className={TEXT_MUTED} />
        </div>
      </div>
    </div>
  );
    ...
    <>
      ...
      {mobileOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className={cx("absolute inset-y-0 left-0 w-72 max-w-[85vw] border-r shadow-2xl shadow-zinc-900/20", BORDER, PANEL_BG)}>
            <div className="flex justify-end p-2">
              <button type="button" onClick={onCloseMobile} className={cx("grid h-11 w-11 place-items-center rounded-full border text-sm font-medium", BORDER, HOVER_BG, TRANSITION, FOCUS)}>
                <X size={18} aria-hidden="true" className={TEXT_MUTED} />
