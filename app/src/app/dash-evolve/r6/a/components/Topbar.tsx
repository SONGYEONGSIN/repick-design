...
import { Bell, HelpCircle, LogOut, Menu, Play, Search, Settings } from "lucide-react";
import Image from "next/image";
import { useEffect, useRef, useState } from "react";
import { CURRENT_USER, unsplashAvatar } from "../lib/data";
import { BORDER, FOCUS_RING, HOVER_ACTIVE_BG, NUM, TEXT_CAPTION, TEXT_PRIMARY, TRANSITION, cx } from "../lib/tokens";

const NOTIFICATIONS = [
  { id: "n1", text: "archive_raw_inventory 태스크가 재시도 2회 후 실패했습니다.", time: "1분 전" },
  { id: "n2", text: "nightly_orders_pipeline 런 #2118이 시작되었습니다.", time: "8분 전" },
  { id: "n3", text: "weekly_ltv_refresh 스케줄이 정상 완료되었습니다.", time: "6시간 전" },
];
  ...
  return (
    <header className={cx("flex h-11 shrink-0 items-center gap-2 border-b px-3 sm:px-4", BORDER, "bg-white dark:bg-zinc-950")}>
      ...
      >
        ...
        <span className="hidden truncate sm:inline">태스크, 파이프라인 검색</span>
        <span
          aria-hidden="true"
      ...
      <div className="ml-auto flex items-center gap-1.5">
        ...
        <div ref={notifRef} className="relative">
          ...
          >
            ...
            <span className="sr-only">알림 (읽지 않음 3개)</span>
          </button>
          {notifOpen ? (
        ...
        <div ref={userRef} className="relative">
          ...
          >
            ...
            />
            <span className={cx("hidden text-sm font-medium sm:inline", NUM, TEXT_PRIMARY)}>{CURRENT_USER.name}</span>
          </button>
          {userOpen ? (
