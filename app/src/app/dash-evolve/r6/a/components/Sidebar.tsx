...
import { BRAND, BrandIcon, CURRENT_USER, ENVIRONMENTS, NAV_SECTIONS, unsplashAvatar } from "../lib/data";
    ...
    <div ref={ref} className="relative px-3 pt-3">
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className={cx("block truncate text-sm font-medium", TEXT_PRIMARY)}>{env.name}</span>
          <span className={cx("block truncate text-xs", TEXT_CAPTION)}>{env.plan}</span>
        </span>
          ...
          {ENVIRONMENTS.map((e) => {
            ...
            return (
              ...
              >
                ...
                {selected && <Check size={16} aria-hidden="true" className="shrink-0 text-violet-600 dark:text-violet-400" />}
              </button>
            );
          })}
        </div>
