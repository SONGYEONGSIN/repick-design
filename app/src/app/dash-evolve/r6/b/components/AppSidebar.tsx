...
import { CURRENT_USER, WORKSPACE, WORKSPACES } from "../lib/data";

const NAV_ITEMS = [
  { id: "leaderboard", label: "Leaderboard", icon: Trophy, active: true },
  { id: "pipeline", label: "Pipeline", icon: Workflow, active: false },
  { id: "coaching", label: "Coaching", icon: GraduationCap, active: false },
  { id: "contests", label: "Contests", icon: Flag, active: false },
  { id: "reports", label: "Reports", icon: BarChart3, active: false },
];

/**
 * Workspace-level app shell sidebar: brand lockup + workspace switcher +
 * primary nav + current user. Every interactive control inside is its own
 * min-h-11 (44px) target, independent of the h-11 header row it sits in.
 */
  ...
  const content = (
    <div className="flex h-full flex-col">
      <div className="flex h-11 items-center gap-2 px-4">
        ...
        <span className="text-sm font-semibold tracking-tight text-zinc-900">Podium</span>
        <button
          type="button"
      ...
      <div className="border-t border-zinc-200 p-3">
        <div className="flex items-center gap-2.5 rounded-lg px-2 py-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-xs font-medium text-zinc-900">{CURRENT_USER.name}</p>
            <p className="truncate text-[11px] text-zinc-500">{CURRENT_USER.role}</p>
          </div>
        </div>
      </div>
    </div>
