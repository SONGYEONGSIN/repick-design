...
function WorkspaceSwitcher() {
  ...
  return (
    <div ref={ref} className="relative px-3 pt-3">
      ...
      >
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-md bg-teal-500/10 text-[11px] font-semibold text-teal-300">{ws.name.slice(0, 1)}</span>
      ...
      {open ? (
        <div role="listbox" aria-label="Select workspace" className={cx("absolute left-3 right-3 top-full z-40 mt-1.5 overflow-hidden rounded-xl border p-1", BORDER, "bg-zinc-900 shadow-lg shadow-black/40")}>
          {WORKSPACES.map((w) => {
            ...
            return (
              <button
                ...
                className={cx(
                  ...
                  TRANSITION,
                  FOCUS_RING,
                  selected ? "bg-teal-500/10" : HOVER_ACTIVE_BG,
                )}
              >
                <span className="min-w-0 flex-1">
                ...
                {selected && <Check size={16} aria-hidden="true" className="shrink-0 text-teal-300" />}
              </button>
            );
          })}
        </div>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-11 shrink-0 items-center gap-2 px-4">
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-[8px] bg-teal-500 text-zinc-950">
            ...
            <ul className="flex flex-col gap-0.5">
              {section.items.map((item) => {
                if (item.disabled) {
                  return (
                    <li key={item.id}>
                      <span aria-disabled="true" className="flex min-h-11 cursor-not-allowed items-center gap-3 rounded-lg px-2.5 py-2 text-sm font-medium text-zinc-400">
                        <item.Icon size={17} aria-hidden="true" />
                        {item.label}
                        <span className="ml-auto rounded-full bg-zinc-800 px-1.5 py-0.5 text-[10px] font-medium text-zinc-400">Soon</span>
                      </span>
                    </li>
                  );
                }
                  ...
                  <li key={item.id}>
                    <a
                      ...
                      className={cx(
                        ...
                        TRANSITION,
                        FOCUS_RING,
                        item.active ? "bg-teal-500/10 text-teal-300" : cx(TEXT_CAPTION, "hover:bg-white/5 hover:text-zinc-100"),
                      )}
                    >
                      <item.Icon size={17} aria-hidden="true" />
                      {item.label}
                      {item.badge ? <span className="ml-auto grid h-5 min-w-5 place-items-center rounded-full bg-rose-500 px-1 text-[11px] font-semibold text-zinc-950">{item.badge}</span> : null}
                    </a>
                  </li>
                );
              })}
