  ...
  return (
    <div ref={ref} className="relative px-3 pt-3">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className={cx(
          "flex h-11 w-full items-center gap-2 rounded-xl border px-2.5 text-left",
          BORDER,
          "bg-white",
          HOVER_ACTIVE_BG,
          TRANSITION,
      ...
      >
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-md bg-indigo-50 text-[11px] font-semibold text-indigo-700">
          {ws.name.slice(0, 1)}
        </span>
      ...
      {open ? (
        <div
          ...
          className={cx("absolute left-3 right-3 top-full z-40 mt-1.5 overflow-hidden rounded-xl border p-1", BORDER, "bg-white shadow-lg")}
        >
          {WORKSPACES.map((w) => {
            ...
            return (
              <button
                ...
                className={cx(
                  ...
                  TRANSITION,
                  FOCUS_RING,
                  selected ? "bg-indigo-50" : HOVER_ACTIVE_BG,
                )}
              >
                <span className="min-w-0 flex-1">
                ...
                {selected && <Check size={16} aria-hidden="true" className="shrink-0 text-indigo-600" />}
              </button>
            );
          })}
        </div>
                ...
                if (item.disabled) {
                  return (
                    <li key={item.id}>
                      <span
                        aria-disabled="true"
                        className="flex min-h-11 cursor-not-allowed items-center gap-3 rounded-lg px-2.5 py-2 text-sm font-medium text-zinc-500"
                      >
                        <item.Icon size={17} aria-hidden="true" />
                        {item.label}
                        <span className="ml-auto rounded-full bg-zinc-100 px-1.5 py-0.5 text-[10px] font-medium text-zinc-500">Soon</span>
                      </span>
                    </li>
                  );
                }
                  ...
                  <li key={item.id}>
                    <a
                      ...
                      className={cx(
                        ...
                        TRANSITION,
                        FOCUS_RING,
                        item.active ? "bg-indigo-50 text-indigo-700" : cx(TEXT_CAPTION, "hover:bg-zinc-100 hover:text-zinc-900"),
                      )}
                    >
                      <item.Icon size={17} aria-hidden="true" />
                      {item.label}
                      {item.badge ? (
                        <span className="ml-auto grid h-5 min-w-5 place-items-center rounded-full bg-indigo-600 px-1 text-[11px] font-semibold text-white">
                          {item.badge}
                        </span>
                      ) : null}
                    </a>
      ...
      <div className={cx("border-t p-3", BORDER)}>
        <div className="flex items-center gap-2.5 rounded-lg px-2 py-2">
          <Image
            ...
            width={28}
            height={28}
            className="h-7 w-7 shrink-0 rounded-full border border-black/5 object-cover"
          />
