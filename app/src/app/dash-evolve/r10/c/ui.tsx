...
/** Progress bar — track + fill; color is always paired with an adjacent numeric label by the caller. */
...
export function SegmentedControl<T extends string>({
  ...
  return (
    <div role="radiogroup" aria-label={ariaLabel} className={cx("inline-flex rounded-lg border p-0.5", BORDER, "bg-zinc-50")}>
      {options.map((opt, idx) => {
        ...
        return (
          <button
            ...
            className={cx(
              ...
              TRANSITION,
              FOCUS_RING,
              checked ? cx("bg-white shadow-sm", TEXT_PRIMARY) : cx(TEXT_CAPTION, "hover:text-zinc-900"),
            )}
          >
            {opt.label}
          </button>
...
/** Deterministic mini sparkline — line + soft area + endpoint. Coordinates rounded to 2 decimals. */
