...
function WorkspaceSwitcher() {
  ...
  return (
    <div ref={ref} className="relative px-3 pt-3">
      ...
      {open ? (
        <div role="listbox" aria-label="Select network" className={cx("absolute left-3 right-3 top-full z-40 mt-1.5 rounded-xl border p-1", BORDER, PANEL_BG, "shadow-xl shadow-zinc-900/10")}>
          {WORKSPACES.map((w) => {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-11 shrink-0 items-center gap-2 px-4">
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-[9px] bg-violet-600 text-white">
          <BRAND.Icon size={16} aria-hidden="true" strokeWidth={2.25} />
        </span>
        {/* Wordmark stays on the global Pretendard font-sans stack — no display typeface assigned this round. */}
