...
export default function CommandPalette({ onClose, onSelectAlert }: { onClose: () => void; onSelectAlert: (id: string) => void }) {
      ...
      <div role="dialog" aria-modal="true" aria-label="Command palette" onClick={(e) => e.stopPropagation()} className={cx("w-full max-w-xl rounded-2xl border shadow-2xl shadow-zinc-900/20", BORDER, PANEL_BG)}>
        <div className={cx("flex items-center gap-2.5 border-b px-3 py-2", BORDER)}>
          ...
          <input
            ...
            type="text"
            placeholder="Search zones and facilities…"
            aria-label="Search zones and facilities"
        ...
        <div className="max-h-[60vh] overflow-y-auto p-2 [scrollbar-width:thin]">
          {results.length === 0 ? <p className={cx("px-2.5 py-6 text-center text-sm font-normal", TEXT_AUX)}>No zones match that.</p> : null}
          {results.length > 0 ? (
            <div>
