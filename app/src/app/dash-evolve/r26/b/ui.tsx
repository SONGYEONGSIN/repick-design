        ...
        return (
          <button
            ...
            className={cx(
              ...
              TRANSITION,
              FOCUS,
              active ? "bg-violet-700 font-semibold text-white" : cx("font-medium", TEXT_MUTED, "hover:bg-white hover:text-zinc-900"),
            )}
          >
            {opt.label}
          </button>
...
export function Sparkline({ values, className }: { values: number[]; className?: string }) {
  const W = 64;
  const H = 20;
  if (values.length < 2) return <svg width={W} height={H} aria-hidden="true" className={className} />;
  const lo = Math.min(...values);
  const hi = Math.max(...values);
  const span = hi - lo || 1;
  const step = W / (values.length - 1);
  const points = values.map((v, i) => `${Math.round(i * step * 100) / 100},${Math.round((H - 2 - ((v - lo) / span) * (H - 4)) * 100) / 100}`).join(" ");
  return (
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} aria-hidden="true" className={className}>
