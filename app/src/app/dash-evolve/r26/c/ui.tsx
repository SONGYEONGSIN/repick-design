        ...
        return (
          <button
            ...
            className={cx(
              ...
              TRANSITION,
              FOCUS,
              active ? cx(ACCENT_SOLID, "font-semibold") : cx("font-medium", TEXT_MUTED, "hover:bg-white hover:text-zinc-900"),
            )}
          >
            {opt.label}
          </button>
...
export function ProgressBar({ pct, label }: { pct: number; label: string }) {
  ...
  return (
    <div>
      <div className="h-2 w-full overflow-hidden rounded-full bg-zinc-100" role="progressbar" aria-label={label} aria-valuenow={Math.round(clamped)} aria-valuemin={0} aria-valuemax={100}>
        <div className="h-full rounded-full bg-violet-600" style={{ width: `${clamped}%` }} />
      </div>
    </div>
  );
}
...
export function Sparkline({ values, ariaLabel }: { values: number[]; ariaLabel: string }) {
  ...
  const points = values
    .map((v, i) => {
      ...
      const y = r2(h - ((v - min) / range) * (h - 4) - 2);
      return `${x},${y}`;
    })
    .join(" ");
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} width={w} height={h} role="img" aria-label={ariaLabel}>
      <polyline points={points} fill="none" stroke="#7c3aed" strokeWidth={1.75} strokeLinecap="round" strokeLinejoin="round" />
      <circle cx={lastX} cy={lastY} r={2.25} fill="#7c3aed" />
    </svg>
  );
}
