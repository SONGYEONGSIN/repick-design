...
/** Mounted only while open — `query` starts fresh on every open by construction, so no effect has to
 *  reset it (sidesteps `react-hooks/set-state-in-effect`). */
export default function CommandPalette({ onClose, onPinCohort }: { onClose: () => void; onPinCohort: (cohortId: string) => void }) {
      ...
      <div role="dialog" aria-modal="true" aria-label="Command palette" onClick={(e) => e.stopPropagation()} className={cx("w-full max-w-xl rounded-2xl border shadow-2xl shadow-black/50", BORDER, PANEL_BG)}>
        <div className={cx("flex items-center gap-2.5 border-b px-3 py-2", BORDER)}>
          ...
          <input
            ...
            type="text"
            placeholder="Search cohorts by month or status…"
            aria-label="Search cohorts by month or status"
        ...
        <div className="max-h-[60vh] overflow-y-auto p-2 [scrollbar-width:thin]">
          {results.length === 0 ? <p className={cx("px-2.5 py-6 text-center text-sm font-normal", TEXT_AUX)}>No cohorts match that.</p> : null}
          {results.length > 0 ? (
            <div>
              ...
              {results.map((e) => (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1 truncate">
                    <span>{e.title}</span>
                    <span className={cx("ml-2 text-[11px] font-normal", TEXT_AUX)}>{`Pin as baseline · ${e.meta}`}</span>
                  </span>
                </button>
              ))}
            </div>
