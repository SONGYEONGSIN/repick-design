        ...
        <div role="listbox" aria-label="Select workspace" className={cx("absolute left-3 right-3 top-full z-40 mt-1.5 rounded-xl border p-1", BORDER, PANEL_BG, "shadow-xl shadow-black/40")}>
          {WORKSPACES.map((w) => {
            ...
            return (
              ...
              >
                <span className="min-w-0 flex-1">
                  <span className={cx("block truncate text-sm font-medium", selected ? "text-emerald-400" : TEXT_PRIMARY)}>{w.name}</span>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-11 shrink-0 items-center gap-2 px-4">
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-[9px] bg-emerald-800 text-white">
