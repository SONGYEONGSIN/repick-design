  ...
  return (
    <header className={cx("sticky top-0 z-30 flex h-11 shrink-0 items-center gap-2 border-b px-3 sm:px-4", BORDER, PANEL_BG)}>
      ...
      >
        ...
        <span className="sr-only truncate sm:not-sr-only sm:inline">Search drivers and line items</span>
        <span aria-hidden="true" className={cx("ml-auto hidden shrink-0 rounded-md border px-1.5 py-0.5 text-[11px] font-medium sm:inline-flex", BORDER, "bg-white/5", TEXT_AUX)}>
          &#8984;K
        </span>
      </button>
      ...
      <div className="ml-auto flex items-center gap-1.5">
        ...
        <div ref={notifRef} className="relative">
          ...
          >
            ...
            <span className="absolute right-2.5 top-2.5 h-1.5 w-1.5 rounded-full bg-lime-400" aria-hidden="true" />
            <span className="sr-only">{`Notifications, ${NOTIFICATIONS.length} unread`}</span>
          </button>
          {notifOpen ? (
            <div role="menu" aria-label="Notifications" className={cx("absolute right-0 top-full z-40 mt-2 w-80 max-w-[calc(100vw-1.5rem)] rounded-xl border", BORDER, "bg-zinc-900 shadow-xl shadow-black/60")}>
        ...
        <div ref={userRef} className="relative">
          ...
          {userOpen ? (
            <div role="menu" aria-label="Account" className={cx("absolute right-0 top-full z-40 mt-2 w-60 rounded-xl border p-1", BORDER, "bg-zinc-900 shadow-xl shadow-black/60")}>
              ...
              <div className="p-1">
                {[
                  ...
                  { label: "Reconciliation docs", Icon: HelpCircle },
