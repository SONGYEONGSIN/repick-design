...
import { BellRing, ChevronsUpDown, Compass, Layers, LayoutDashboard, LayoutGrid, Settings } from "lucide-react";
...
const NAV_ITEMS: NavItem[] = [
  { label: "리포트", Icon: LayoutGrid, current: true },
  { label: "대시보드", Icon: LayoutDashboard },
  { label: "탐색", Icon: Compass },
  { label: "알림 규칙", Icon: BellRing },
  { label: "설정", Icon: Settings },
];

export default function GlobalSidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
    ...
    <>
      ...
      >
        ...
        <div className="flex h-[44px] shrink-0 items-center gap-2 px-4 pt-4">
          ...
          <p className="text-[15px] font-semibold tracking-tight text-zinc-900">Parallax</p>
        </div>
        ...
        <div className="px-3 pt-4">
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block truncate text-sm font-medium text-zinc-800">노스브릿지 프로덕트팀</span>
              <span className="block truncate text-xs text-zinc-500">Growth 플랜 · 리포트 9개</span>
            </span>
              ...
              <li key={item.label}>
                <button
                  ...
                  className={`flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-sm font-medium transition-colors motion-reduce:transition-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500 ${
                    item.current ? "bg-indigo-50 text-indigo-700" : "text-zinc-600 hover:bg-zinc-50 hover:text-zinc-900"
                  }`}
                >
        ...
        <div className="flex shrink-0 items-center gap-2.5 border-t border-zinc-200 px-3 py-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-medium text-zinc-800">노유진</p>
            <p className="truncate text-xs text-zinc-500">yujin.no@parallax.io</p>
          </div>
        </div>
      </aside>
    </>
