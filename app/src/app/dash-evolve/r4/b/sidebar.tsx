import { Radar, LayoutGrid, AlertTriangle, TrendingUp, Plug, BellRing, Settings, X } from "lucide-react";
import { cn, FOCUS_RING } from "./cn";
import { Avatar, Popover } from "./ui";

const NAV_ITEMS = [
  { label: "Service health", icon: LayoutGrid, active: true },
  { label: "Incidents", icon: AlertTriangle, active: false },
  { label: "Uptime reports", icon: TrendingUp, active: false },
      ...
      >
        <div className="flex h-[44px] items-center justify-between gap-2 px-4 pt-4">
          <div className="flex items-center gap-2">
            <span className="flex size-7 items-center justify-center rounded-md bg-violet-400/15 text-violet-300">
              <Radar aria-hidden="true" className="size-4" />
            </span>
            <span className="text-sm font-semibold tracking-tight text-zinc-50">Wardline</span>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-3 pt-5">
          <Popover
            label={<span className="truncate">Solace Systems</span>}
            align="left"
            fixedHeight={false}
          ...
          >
            {(close) => (
              <div className="py-1">
                ...
                {["Solace Systems", "Solace Systems — Sandbox"].map((ws) => (
                  <button
                    key={ws}
                    type="button"
                    onClick={close}
          ...
          <Popover
            label={
              <span className="flex min-w-0 items-center gap-2.5">
                <Avatar name="Renna Osei" size={28} />
                <span className="flex min-w-0 flex-col items-start leading-tight">
                  <span className="truncate text-sm font-medium text-zinc-100">Renna Osei</span>
                  <span className="truncate text-xs text-zinc-400">Platform reliability</span>
                </span>
              </span>
            }
            align="left"
