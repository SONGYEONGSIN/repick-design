...
import { Menu, Search, Bell, Download, ChevronDown, Settings, LogOut, UserRound } from "lucide-react";
...
const INITIAL_NOTIFICATIONS: Notification[] = [
  { id: "n1", title: "Lakeside is at 96% capacity", body: "Tomorrow's schedule is nearly full — consider opening an overflow slot.", read: false },
  { id: "n2", title: "Staffing gap on Sep 25", body: "Only 4 of 5 needed providers are scheduled at Downtown.", read: false },
  { id: "n3", title: "Weekly capacity digest ready", body: "September utilization averaged 71%, up 4.2 pts on August.", read: true },
];

export function Topbar({
  onOpenMenu, onOpenSearch, onExport,
}: { onOpenMenu: () => void; onOpenSearch: () => void; onExport: () => void }) {
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-zinc-200 bg-white/95 px-4 backdrop-blur sm:px-6 lg:px-8">
      ...
      >
        ...
        <span className="sr-only sm:not-sr-only truncate text-sm font-normal">Search or run a command</span>
      ...
      <div className="ml-auto flex items-center gap-2">
        <button
          type="button"
          onClick={onExport}
          className={`flex h-11 items-center gap-2 rounded-lg bg-orange-700 px-3.5 text-sm font-medium text-white hover:bg-orange-800 ${FOCUS_RING}`}
        >
          <Download aria-hidden="true" className="h-4.5 w-4.5 shrink-0" />
    ...
    <div className="relative" ref={ref}>
      ...
      >
        ...
        {unread > 0 && (
          <span
            aria-hidden="true"
            className="absolute right-2 top-2 flex h-4 min-w-4 items-center justify-center rounded-full bg-orange-600 px-1 text-[10px] font-bold text-white"
          >
            {unread}
          </span>
        )}
        ...
        >
          <div className="flex items-center justify-between border-b border-zinc-200 px-4 py-3">
            ...
            <button
              ...
              className={`rounded-md px-2 py-2 text-xs font-medium text-orange-700 hover:underline ${FOCUS_RING}`}
            >
              Mark all read
            </button>
          </div>
            ...
            {items.map((n) => (
              <li key={n.id} className="border-b border-zinc-100 px-4 py-3 last:border-b-0">
                <div className="flex items-start gap-2">
                  {!n.read && <span aria-hidden="true" className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-orange-600" />}
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <span className="sr-only sm:not-sr-only text-sm font-medium text-zinc-900">Ava Morrow</span>
      ...
      {open && (
        ...
        >
          <div className="border-b border-zinc-200 px-4 py-3">
            <p className="truncate text-sm font-medium text-zinc-900">Ava Morrow</p>
            <p className="truncate text-xs font-normal text-zinc-500">ava.morrow@brightpath.example</p>
          </div>
          <button
            type="button"
            role="menuitem"
