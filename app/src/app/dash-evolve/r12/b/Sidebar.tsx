...
import { Check, ChevronsUpDown, FileSignature, SlidersHorizontal, X } from "lucide-react";
...
function WorkspaceSwitcher() {
  ...
  return (
    <div ref={ref} className="relative px-3 pt-3">
      ...
      {open ? (
        <div role="listbox" aria-label="Select workspace" className={cx("absolute left-3 right-3 top-full z-40 mt-1.5 overflow-hidden rounded-xl border p-1", BORDER, "bg-white dark:bg-zinc-900 shadow-lg")}>
          {WORKSPACES.map((w) => {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-11 shrink-0 items-center gap-2 px-4">
        <span className={cx("grid h-7 w-7 shrink-0 place-items-center rounded-[8px] text-white", "bg-sky-700 dark:bg-sky-500 dark:text-zinc-950")}>
          <FileSignature size={16} aria-hidden="true" strokeWidth={2.25} />
        </span>
                  ...
                  return (
                    <li key={item.id}>
                      <span aria-disabled="true" className={cx("flex min-h-11 cursor-not-allowed items-center gap-3 rounded-lg px-2.5 py-2 text-sm font-medium", TEXT_CAPTION)}>
                        ...
                        {item.label}
                        <span className={cx("ml-auto rounded-full px-1.5 py-0.5 text-[10px] font-medium", "bg-zinc-100 dark:bg-zinc-800", TEXT_CAPTION)}>Soon</span>
                      </span>
                    </li>
                  );
                }
                  ...
                  <li key={item.id}>
                    <a
                      ...
                      className={cx(
                        ...
                        TRANSITION,
                        FOCUS_RING,
                        item.active ? cx(ACCENT_SUBTLE, "font-semibold") : cx(TEXT_CAPTION, "hover:bg-zinc-50 hover:text-zinc-900 dark:hover:bg-white/5 dark:hover:text-zinc-100"),
                      )}
                    >
                      <item.Icon size={17} aria-hidden="true" />
                      {item.label}
