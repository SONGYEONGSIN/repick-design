    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center px-4 pt-24 sm:pt-32">
      ...
      >
        <div className="flex items-center gap-2.5 border-b border-zinc-100 px-4">
          ...
          {/*
            FOCUS-VISIBILITY FIX (this is the rule the previous candidate
            broke): this input carries FOCUS_RING and nothing else touches
            outline on it or on any ancestor. The earlier version paired an
            `outline-none` utility directly on this same element with the
            `focus-visible:outline` utilities — a dead idiom in this
            Tailwind v4 setup, where an `outline-none` anywhere in the
            class list can zero out `--tw-outline-style` and cancel a
            later `focus-visible:outline` on the very same element, so tabbing
            into the palette's search field painted no visible focus ring at
            all despite the "correct" classes being present. Modal-only
            controls like this one are invisible to the default-render a11y
            scan, so this exact spot is verified by hand: there is no
            `outline-none` utility anywhere in this file, on this input, or
            on any of its ancestors — only the direct, additive
            `focus-visible:outline-2 focus-visible:outline-offset-2
            focus-visible:outline-cyan-600` from FOCUS_RING, which paints a
            real 2px outline on Tab with no earlier rule to cancel it.
          */}
          <input
            ref={inputRef}
            value={query}
            ...
            className={cn(
              "h-12 w-full bg-transparent text-sm text-zinc-900 placeholder:text-zinc-500",
              FOCUS_RING,
            )}
          />
