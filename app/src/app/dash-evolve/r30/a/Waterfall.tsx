...
import { cn, FOCUS_RING, PIN_OUTLINE } from "./ui";
...
export function Waterfall({
  ...
  // Ephemeral hover/focus preview — local state only, reverts on
  // leave/blur, and never persists across a render. Kept deliberately
  // separate from `pinnedId` (the persistent axis owned by the parent)
  // so a hover never mutates or is mistaken for a pin.
            ...
            <div className="absolute inset-0">
              {bars.map((bar) => {
                ...
                return (
                  ...
                  // Focus uses the safe outline pattern directly (see FOCUS_RING);
                  // no `outline-none` is set anywhere on this button or an ancestor,
                  // so the focus-visible outline below is never cancelled.
                  <button
                    key={bar.id}
                    type="button"
                  ...
                  >
                    <span
                      ...
                      className={cn(
                        ...
                        // Permanent "pinned" mark — an explicit outline, never
                        // `ring`/`ring-offset` (which can paint fully transparent
                        // in this Tailwind v4 setup); paired with the floating pin
                        // badge below as a second, redundant visible cue.
                        pinned && PIN_OUTLINE,
                      )}
                    />
                    {pinned && (
              ...
              {/* Delta / total labels above each bar — always visible, never hover-only */}
              {bars.map((bar) => (
                <div
                  key={`${bar.id}-label`}
                  aria-hidden="true"
              ...
              {/* Hover/focus crosshair tooltip — ephemeral, reverts on leave/blur */}
              {hoveredBar && (
                <div
                  aria-hidden="true"
                  style={{
