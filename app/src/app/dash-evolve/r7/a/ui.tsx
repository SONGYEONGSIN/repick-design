...
import { STAGE_META, round2, type StageId } from "./data";
import {
  BORDER,
  CARD,
  FOCUS_RING,
...
export function StageBadge({ stage }: { stage: StageId }) {
  ...
  return (
    ...
    >
      <span aria-hidden="true" className={cx("h-1.5 w-1.5 shrink-0 rounded-full", m.chipDot)} />
      {m.label}
    </span>
  );
}

/** 컴팩트 단계 태그(dot + 라벨) — 좁은 테이블 셀의 from→to 표기용. */
export function StageTag({ stage }: { stage: StageId }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 whitespace-nowrap">
      ...
      <span className={cx("text-xs font-medium", m.text)}>{m.label}</span>
    </span>
  );
}
...
/** 결정론 헬스 히스토리 스파크라인 — 라인 + 은은한 면적 + 종점. 좌표 소수 2자리. */
