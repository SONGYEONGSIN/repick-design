/**
 * Apogee — 라우트 스코프 디자인 토큰. 전역 테마를 건드리지 않고 이 라우트 안에서만 쓰는 클래스 상수.
 * 라이트 = 순백 캔버스(white/zinc-50) + zinc-200 헤어라인 + shadow-sm.
 * 다크 = zinc-950/900 표면 + white/10 보더. 연극적 발광·스캔라인 없음.
 * 대비 규칙: 라이트 표면 보조 텍스트는 zinc-500 이상, 다크 표면은 zinc-400 이상만. (zinc-400 on white 금지)
 * 강조 1색 = indigo (브랜드). 라이프사이클 단계는 데이터-범주 색상으로 STAGE_META(=data.ts)가 담당.
 */
...
/** 로그·코드성 데이터 전용 모노(레이아웃 전역 font-mono, next/font 추가 없음). */
