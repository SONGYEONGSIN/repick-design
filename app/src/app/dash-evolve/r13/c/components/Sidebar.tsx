...
import { Check, ChevronsUpDown, Newspaper, SlidersHorizontal, X } from "lucide-react";
import Image from "next/image";
import { useState } from "react";
import { BRAND, CURRENT_USER, NAV_SECTIONS, WORKSPACES, unsplashAvatar } from "../data";
import { ACCENT_SUBTLE, ACCENT_TEXT, BORDER, DISPLAY_FONT_STYLE, FOCUS_RING, HOVER_ACTIVE_BG, TEXT_CAPTION, TEXT_PRIMARY, TRANSITION, cx } from "./tokens";
  ...
  return (
    <div ref={ref} className="relative px-3 pt-3">
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className={cx("flex h-11 w-full items-center gap-2 rounded-xl border px-2.5 text-left", BORDER, "bg-zinc-900", HOVER_ACTIVE_BG, TRANSITION, FOCUS_RING)}
      >
      ...
      {open ? (
        <div role="listbox" aria-label="Select workspace" className={cx("absolute left-3 right-3 top-full z-40 mt-1.5 overflow-hidden rounded-xl border p-1", BORDER, "bg-zinc-900 shadow-lg")}>
          {WORKSPACES.map((w) => {
            ...
            return (
              <button
                ...
                className={cx("flex min-h-11 w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-sm", TRANSITION, FOCUS_RING, selected ? ACCENT_SUBTLE : HOVER_ACTIVE_BG)}
              >
                <span className="min-w-0 flex-1">
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-11 shrink-0 items-center gap-2 px-4">
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-[8px] bg-orange-500 text-zinc-950">
          <Newspaper size={16} aria-hidden="true" strokeWidth={2.25} />
        </span>
        <span style={DISPLAY_FONT_STYLE} className={cx("text-[15px] font-semibold leading-none tracking-tight", TEXT_PRIMARY)}>
          {BRAND.name}
        </span>
      </div>
