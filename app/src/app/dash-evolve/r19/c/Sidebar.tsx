...
import { ACCENT_SUBTLE, ACCENT_TEXT, BORDER, FOCUS, HOVER_BG, PANEL_BG, SURFACE_INSET, TEXT_AUX, TEXT_AUX_MUTED, TEXT_PRIMARY, TRANSITION, cx } from "./tokens";
  ...
  return (
    <div ref={ref} className="relative px-3 pt-3">
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className={cx("flex h-11 w-full items-center gap-2 rounded-xl border px-2.5 text-left", BORDER, "bg-white", HOVER_BG, TRANSITION, FOCUS)}
      >
      ...
      {open ? (
        <div role="listbox" aria-label="Select workspace" className={cx("absolute left-3 right-3 top-full z-40 mt-1.5 rounded-xl border p-1", BORDER, "bg-white shadow-xl shadow-zinc-900/10")}>
          {WORKSPACES.map((w) => {
                  ...
                  return (
                    <li key={item.id}>
                      <span aria-disabled="true" className={cx("flex min-h-11 cursor-not-allowed items-center gap-3 rounded-lg px-2.5 py-2 text-sm font-medium", TEXT_AUX)}>
                        ...
                        {item.label}
                        <span className={cx("ml-auto rounded-full border px-1.5 py-0.5 text-[10px] font-medium", BORDER, SURFACE_INSET, TEXT_AUX_MUTED)}>Soon</span>
                      </span>
                    </li>
                  );
                }
                  ...
                  <li key={item.id}>
                    <a
                      ...
                      className={cx(
                        ...
                        TRANSITION,
                        FOCUS,
                        item.active ? cx(ACCENT_SUBTLE, "font-semibold") : cx("font-medium", TEXT_AUX_MUTED, "hover:bg-zinc-100 hover:text-zinc-900"),
                      )}
                    >
                      <item.Icon size={17} aria-hidden="true" />
                      {item.label}
