...
import { ACCENT_SOLID, BORDER, FOCUS, HOVER_BG, PANEL_BG, SURFACE_INSET, TEXT_AUX, TEXT_AUX_MUTED, TEXT_PRIMARY, TRANSITION, cx } from "./tokens";
  ...
  return (
    <header className={cx("sticky top-0 z-30 flex h-11 shrink-0 items-center gap-2 border-b px-3 sm:px-4", BORDER, PANEL_BG)}>
      ...
      <button
        ...
        className={cx("ml-1 flex h-11 flex-1 items-center gap-2 rounded-lg border pl-3 pr-2.5 text-left text-sm font-normal sm:max-w-md", BORDER, SURFACE_INSET, HOVER_BG, TEXT_AUX_MUTED, TRANSITION, FOCUS)}
      >
        <Search size={16} aria-hidden="true" className="shrink-0" />
        <span className="sr-only truncate sm:not-sr-only sm:inline">Search queues, windows or a ticket</span>
        <span aria-hidden="true" className={cx("ml-auto hidden shrink-0 rounded-md border px-1.5 py-0.5 text-[11px] font-medium sm:inline-flex", BORDER, "bg-white", TEXT_AUX_MUTED)}>
          &#8984;K
        </span>
      </button>
      ...
      <div className="ml-auto flex items-center gap-1.5">
        ...
        <div ref={notifRef} className="relative">
          ...
          >
            <Bell size={17} aria-hidden="true" className={TEXT_AUX_MUTED} />
          ...
          {notifOpen ? (
            <div role="menu" aria-label="Notifications" className={cx("absolute right-0 top-full z-40 mt-2 w-80 max-w-[calc(100vw-1.5rem)] rounded-xl border", BORDER, "bg-white shadow-xl shadow-zinc-900/10")}>
              <div className={cx("border-b px-4 py-3", BORDER)}>
                <p className={cx("text-sm font-semibold", TEXT_PRIMARY)}>Notifications</p>
              </div>
              <ul className={cx("divide-y", "divide-zinc-100")}>
                {NOTIFICATIONS.map((n) => (
        ...
        <div ref={userRef} className="relative">
          ...
          {userOpen ? (
            <div role="menu" aria-label="Account" className={cx("absolute right-0 top-full z-40 mt-2 w-60 rounded-xl border p-1", BORDER, "bg-white shadow-xl shadow-zinc-900/10")}>
              ...
              <div className="p-1">
                {[
                  ...
                  { label: "SLA documentation", Icon: HelpCircle },
                  ...
                  >
                    <item.Icon size={16} aria-hidden="true" className={TEXT_AUX_MUTED} />
                    {item.label}
                  </button>
                ))}
              </div>
