...
import { ACCENT_SUBTLE, BORDER, CARD, FOCUS_RING, NUM, TEXT_CAPTION, TEXT_PRIMARY, TEXT_SECONDARY, TONE, TRANSITION, cx, type Tone } from "./tokens";
...
/** Dot indicator — always paired with label text elsewhere to honor the color+text rule. */
        ...
        return (
          <button
            ...
            className={cx(
              ...
              TRANSITION,
              FOCUS_RING,
              checked ? cx("bg-white shadow-sm dark:bg-zinc-800", TEXT_PRIMARY) : cx(TEXT_SECONDARY, "hover:text-zinc-900 dark:hover:text-zinc-100"),
            )}
          >
            {opt.label}
          </button>
    ...
    <div ref={ref} className="relative">
      <button
        ...
        className={cx(
          ...
          BORDER,
          "bg-white hover:bg-zinc-50 dark:bg-zinc-950 dark:hover:bg-white/5",
          TEXT_CAPTION,
          TRANSITION,
          FOCUS_RING,
        )}
      ...
      {open ? (
        <div
          ...
          className={cx("absolute left-0 top-full z-30 mt-1.5 min-w-[11rem] overflow-hidden rounded-xl border p-1 shadow-lg", BORDER, "bg-white dark:bg-zinc-900 dark:shadow-black/40")}
        >
          {options.map((opt) => {
            ...
            return (
              <button
                ...
                className={cx(
                  ...
                  TRANSITION,
                  FOCUS_RING,
                  selected ? cx(ACCENT_SUBTLE) : cx(TEXT_CAPTION, "hover:bg-zinc-50 hover:text-zinc-900 dark:hover:bg-white/5 dark:hover:text-zinc-100"),
                )}
              >
                {opt.label}
...
export function SortIcon({ direction }: { direction: "ascending" | "descending" | "none" }) {
  return (
    <span aria-hidden="true" className={cx("inline-flex flex-col leading-none", TEXT_CAPTION)}>
      <svg width="8" height="6" viewBox="0 0 8 6" className={direction === "ascending" ? "opacity-100" : "opacity-30"}>
        <path d="M4 0 L8 6 L0 6 Z" fill="currentColor" />
      </svg>
      <svg width="8" height="6" viewBox="0 0 8 6" className={cx("mt-0.5", direction === "descending" ? "opacity-100" : "opacity-30")}>
