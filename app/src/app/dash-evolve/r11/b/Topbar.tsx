...
import { ACCENT_SOLID, BORDER, FOCUS_RING, HOVER_ACTIVE_BG, TEXT_CAPTION, TEXT_PRIMARY, TRANSITION, cx } from "./tokens";
import { CURRENT_USER, unsplashAvatar } from "./data";
import { useOutsideClose } from "./ui";

const NOTIFICATIONS = [
  { id: "n1", text: "Expansion revenue crossed $140K this month — 3 accounts upgraded tier.", time: "12m ago" },
  { id: "n2", text: "Churn risk: Ashgrove Interiors flagged for cancellation review.", time: "1h ago" },
  { id: "n3", text: "Q2 close reconciliation completed — ending ARR verified against the billing ledger.", time: "3h ago" },
];
  ...
  return (
    <header className={cx("flex h-11 shrink-0 items-center gap-2 border-b px-3 sm:px-4", BORDER, "bg-white dark:bg-zinc-950")}>
      ...
      <button
        ...
        className={cx("ml-1 flex h-11 flex-1 items-center gap-2 rounded-lg border pl-3 pr-2.5 text-left text-sm sm:max-w-sm", BORDER, "bg-zinc-50 hover:bg-zinc-100 dark:bg-zinc-900 dark:hover:bg-zinc-800", TEXT_CAPTION, TRANSITION, FOCUS_RING)}
      >
        <Search size={16} aria-hidden="true" className="shrink-0" />
        <span className="hidden truncate sm:inline">Search drivers, segments, accounts</span>
      ...
      <div className="ml-auto flex items-center gap-1.5">
        ...
        <div ref={notifRef} className="relative">
          ...
          >
            ...
            <span className="absolute right-2.5 top-2.5 h-2 w-2 rounded-full bg-rose-600" aria-hidden="true" />
          </button>
          {notifOpen ? (
            <div role="menu" aria-label="Notifications" className={cx("absolute right-0 top-full z-40 mt-2 w-80 max-w-[calc(100vw-1.5rem)] overflow-hidden rounded-xl border shadow-lg", BORDER, "bg-white dark:bg-zinc-900 dark:shadow-black/40")}>
        ...
        <div ref={userRef} className="relative">
          ...
          {userOpen ? (
            <div role="menu" aria-label="Account" className={cx("absolute right-0 top-full z-40 mt-2 w-60 overflow-hidden rounded-xl border p-1 shadow-lg", BORDER, "bg-white dark:bg-zinc-900 dark:shadow-black/40")}>
