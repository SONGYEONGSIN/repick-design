  ...
  return (
    <div ref={ref} className="relative px-3 pt-3">
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className={cx("flex h-11 w-full items-center gap-2 rounded-xl border px-2.5 text-left", BORDER, "bg-white dark:bg-zinc-900", HOVER_ACTIVE_BG, TRANSITION, FOCUS_RING)}
      >
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-md bg-[#A16207]/10 text-[11px] font-semibold text-amber-700 dark:text-amber-400">{ws.name.slice(0, 1)}</span>
      ...
      {open ? (
        <div role="listbox" aria-label="Select workspace" className={cx("absolute left-3 right-3 top-full z-40 mt-1.5 overflow-hidden rounded-xl border p-1 shadow-lg", BORDER, "bg-white dark:bg-zinc-900 dark:shadow-black/40")}>
          {WORKSPACES.map((w) => {
            ...
            return (
              ...
              >
                ...
                {selected && <Check size={16} aria-hidden="true" className="shrink-0 text-amber-700 dark:text-amber-400" />}
              </button>
            );
          })}
        </div>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-11 shrink-0 items-center gap-2 px-4">
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-[8px] bg-[#0F172A] text-white dark:bg-white dark:text-zinc-950">
            ...
            <ul className="flex flex-col gap-0.5">
              {section.items.map((item) => {
                if (item.disabled) {
                  return (
                    <li key={item.id}>
                      <span aria-disabled="true" className="flex min-h-11 cursor-not-allowed items-center gap-3 rounded-lg px-2.5 py-2 text-sm font-medium text-zinc-400 dark:text-zinc-500">
                  ...
                  <li key={item.id}>
                    <a
                      ...
                      className={cx(
                        ...
                        FOCUS_RING,
                        item.active
                          ? "bg-[#A16207]/10 text-amber-800 dark:bg-amber-500/10 dark:text-amber-300"
                          : cx(TEXT_CAPTION, "hover:bg-zinc-100 hover:text-zinc-900 dark:hover:bg-white/5 dark:hover:text-zinc-100"),
                      )}
                    >
                      <item.Icon size={17} aria-hidden="true" />
                      {item.label}
                      {item.badge ? <span className="ml-auto grid h-5 min-w-5 place-items-center rounded-full bg-rose-600 px-1 text-[11px] font-semibold text-white">{item.badge}</span> : null}
                    </a>
                  </li>
                );
              })}
