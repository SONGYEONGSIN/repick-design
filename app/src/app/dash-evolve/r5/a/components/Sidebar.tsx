...
import { Anchor, Check, ChevronsUpDown, Settings, X } from "lucide-react";
import Image from "next/image";
import { useEffect, useRef, useState } from "react";
import { BRAND, CURRENT_USER, ENTITIES, NAV_SECTIONS, unsplashAvatar } from "../lib/data";
import { BORDER, FOCUS_RING, HOVER_ACTIVE_BG, TEXT_CAPTION, TEXT_PRIMARY, TRANSITION, cx } from "../lib/tokens";
  ...
  return (
    <div ref={ref} className="relative px-3 pt-3">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className={cx(
          "flex min-h-11 w-full items-center gap-2 rounded-xl border px-2.5 py-2 text-left",
          BORDER,
      ...
      >
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-md bg-blue-50 text-[11px] font-semibold text-blue-700 dark:bg-blue-500/10 dark:text-blue-300">
          {entity.name.slice(0, 1)}
        </span>
        <span className="min-w-0 flex-1">
          <span className={cx("block truncate text-sm font-medium", TEXT_PRIMARY)}>{entity.name}</span>
          <span className={cx("block truncate text-xs", TEXT_CAPTION)}>{entity.plan}</span>
        </span>
            ...
            return (
              <button
                ...
                className={cx(
                  ...
                  TRANSITION,
                  FOCUS_RING,
                  selected ? "bg-blue-50 dark:bg-blue-500/10" : HOVER_ACTIVE_BG,
                )}
              >
                <span className="min-w-0 flex-1">
                  <span className={cx("block truncate font-medium", TEXT_PRIMARY)}>{e.name}</span>
                  <span className={cx("block truncate text-xs", TEXT_CAPTION)}>{e.plan}</span>
                </span>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-11 shrink-0 items-center gap-2 px-4">
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-[8px] bg-blue-600 text-white">
          <Anchor size={16} aria-hidden="true" strokeWidth={2.25} />
        </span>
                ...
                if (item.disabled) {
                  return (
                    <li key={item.id}>
                      <span
                        aria-disabled="true"
                        className="flex min-h-11 cursor-not-allowed items-center gap-3 rounded-lg px-2.5 py-2 text-sm font-medium text-zinc-500 dark:text-zinc-400"
                      >
                ...
                return (
                  <li key={item.id}>
                    ...
                    >
                      ...
                      {item.label}
                      {item.badge ? (
                        <span className="ml-auto grid h-5 min-w-5 place-items-center rounded-full bg-blue-600 px-1 text-[11px] font-semibold text-white">
                          {item.badge}
                        </span>
                      ) : null}
                    </a>
