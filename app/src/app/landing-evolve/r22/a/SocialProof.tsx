...
import { ACCENT_BRIGHT } from "./Hero";

export function SocialProof() {
  return (
    <section className="border-b border-[#1C1C22] bg-[#0B0B0F] px-6 py-20 sm:px-10 lg:px-16">
      <div className="mx-auto max-w-[1320px]">
        <h2 className="text-[11px] font-semibold uppercase text-[#A1A1AA]" style={{ letterSpacing: "0.28em" }}>
          Fig. 04 — In the field
        </h2>
        ...
        <div className="mt-16 grid grid-cols-1 gap-6 lg:grid-cols-3">
          {TESTIMONIALS.map((t, i) => (
            <Reveal key={t.name} delay={i * 0.08} className="flex min-w-0 flex-col rounded-2xl border border-[#1C1C22] bg-[#111116] p-6">
              <Quote className="h-5 w-5" style={{ color: ACCENT_BRIGHT }} aria-hidden="true" />
              <p className="mt-4 flex-1 text-[14px] font-normal leading-[1.6] text-[#D4D4D8]">&ldquo;{t.quote}&rdquo;</p>
