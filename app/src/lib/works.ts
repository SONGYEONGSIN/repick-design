...
export const LAST_UPDATED = "2026-09-01"; // determinism rule: no dynamic Date calls — update this by hand when refreshing

export const NATIVE_WORKS: Work[] = [
  { id: "n20", route: "/native-app/index.html?screen=relist", brand: "Bulk Relist", desc: { en: "A contextual bottom bar that does not exist until something is selected — it appears at selection>0 with a live-region count, is mutually exclusive with the post-action undo overlay, and is a third band form distinct from both the blocked state machine and the standing action bar.", ko: "선택하기 전에는 존재하지 않는 컨텍스추얼 하단 바 — 선택 1 이상에서 라이브 리전 카운트와 함께 등장하고 사후 undo 오버레이와 상호배타다. 막힌 상태기계·상시 액션바와 구별되는 세 번째 밴드 형태 (auto-native-r16 승자, 2:1)" }, target: "native", category: "mobile" },
...
export const LANDING_WORKS: Work[] = [
  { id: "v21", route: "/v21", brand: "Trust Score Console", desc: { en: "Four weighted sliders feeding one radial gauge that renormalizes live — moving any single slider redistributes all four contributions, and the closing CTA quotes the resulting number verbatim rather than restating a fixed claim.", ko: "4개 가중치 슬라이더가 하나의 레이더 게이지를 실시간 재정규화한다 — 슬라이더 하나만 움직여도 네 기여값이 전부 재분배되고, 클로징 CTA 가 그 숫자를 문자 그대로 인용한다 (자동 landing r18 승자, 3:0 만장일치)" }, target: "landing", category: "landing" },
  { id: "v20", route: "/v20", brand: "Assay", desc: { en: "A verification pipeline where one category toggle drives a sequential gate chain — a switched-off category is marked Skipped in the pipeline and the trust score, the tab scores, four aggregate cards and the evidence panel all recompute from what remains, rather than merely hiding content.", ko: "카테고리 토글 하나가 순차 게이트 체인을 구동하는 검증 파이프라인 — 꺼진 카테고리는 파이프라인에서 Skipped 로 표시되고 신뢰 점수·탭 점수·집계 카드 4개·증거면이 남은 것만으로 전부 재계산된다. 콘텐츠를 숨기는 토글과 다른 축이다 (자동 landing r17 승자, 2:1)" }, target: "landing", category: "landing" },
...
export const DASH_LAB_WORKS: Work[] = [
  { id: "d53", route: "/dash/d53", brand: "Harborline", desc: { en: "A master-detail support console where clicking pins persistently with a visible badge and hovering previews with nothing persisted — the two modes are labelled differently so a reader can tell which one they are in without reading the source.", ko: "클릭은 가시 배지를 남기는 영속 pin, 호버는 아무것도 남기지 않는 프리뷰인 마스터-디테일 지원 콘솔 — 두 모드에 다른 라벨을 붙여 소스를 읽지 않고도 어느 쪽인지 알 수 있다 (자동 dash r24 승자, 2:1)" }, target: "dash", category: "dashboard" },
  { id: "d52", route: "/dash/d52", brand: "Floorline", desc: { en: "A three-pane comp terminal that splits selection three ways — an active id that recomputes the chart at once, a pinned feed id that moves only via an explicit Pin button with an out-of-sync banner exposing the gap, and a hover state that persists nothing.", ko: "선택 파급을 셋으로 가른 3페인 비교 터미널 — 즉시 차트를 재계산하는 activeId, 명시적 Pin 버튼으로만 움직이며 그 간극을 out-of-sync 배너로 드러내는 pinnedFeedId, 아무것도 남기지 않는 hover (자동 dash r23 승자, 2:1)" }, target: "dash", category: "dashboard" },
  { id: "d51", route: "/dash/d51", brand: "Redoubt", desc: { en: "A security audit log where the dominant visualization is a vertical chronology — filtering partially recomputes, clicking an event opens a non-persistent inspector, and the split between the two is spelled out in a code comment rather than left implicit.", ko: "세로 연대기 이벤트 스트림이 지배 시각화인 보안 감사 로그 — 필터는 부분 재계산, 이벤트 클릭은 비영속 인스펙터이고, 그 분리를 코드 주석에 명시했다 (자동 dash r22 승자, 생존 1후보 단독 심사 후 정제)" }, target: "dash", category: "dashboard" },
