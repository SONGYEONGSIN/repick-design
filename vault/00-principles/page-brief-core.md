...
> 타입 목록의 정본은 코드다 — `app/src/lib/works.ts`의 **`PAGE_TYPES`** 배열이 카테고리 유니온이자 자율 라운드의 생성 큐이며, 순서가 곧 우선순위다. 아래 표는 **프로파일 문서가 있는 타입**만 추린 것이라 `PAGE_TYPES`보다 짧다. 프로파일이 없는 타입이 큐에서 뽑히면 그 라운드가 [[page-brief-core]]만으로 생성하고, 그때 나온 delta가 새 프로파일의 초안이 된다.
...
| `careers` | [[brief-careers]] | 3종 | 허용 (§5 제약 준수) |
