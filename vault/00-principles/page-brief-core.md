...
| ↳ **상태를 열어야 닿는 것**(⌘K 팔레트·뷰 토글 뒤) | `focus` | 하드페일 (2026-08-17 승격) |
...
- 포커스 가시(`outline-none` 단독 금지 — 반드시 `focus-visible` 링과 함께)
  ...
  **2026-08-17 하드페일로 승격했다.** 켠 날은 기록만이었고 3작품 11요소가 걸렸다 — `d45` 팔레트 옵션 8 · `d33`·`d29` 팔레트 입력 각 1. 셋을 고쳐 소급 44라우트 0 으로 내린 뒤 올렸다. **상태 뒤라고 덜 중요한 결함이 아니다 — 팔레트는 ⌘K 한 번이면 열린다.**
  - `d33`·`d29` 는 앞서 11작품에서 고친 **그 결함**이었다(팔레트 입력에 링이 없다). 팔레트가 닫혀 있어 안 보였을 뿐이고, 상태를 여니 바로 나왔다.
  - `d45` 는 **콤보박스 옵션이 탭 순서에 있었다.** `tabIndex={-1}` 로 뺐다 — 포커스는 입력에 머물고 활성 항목은 `aria-activedescendant` 가 가리킨다. **`tabIndex` 만 빼면 안 되는 경우가 있다**: `aria-activedescendant` 가 없는 곳에서 그러면 스크린리더가 활성 항목을 아예 못 읽는다. 패턴을 완결해야 한다.
  **소스로 세면 14작품, 렌더로 재면 3작품이었다** — 나머지는 포커스 시 무언가가 실제로 그려진다. 정본에 14 를 적었다가 소급 스캔이 반증해 고쳤다. **판정은 렌더가 한다.**
...
- **`aria-hidden` 은 `color-contrast` 면제가 아니다 (2026-08-24 편입).** axe 는 **보이는 텍스트 노드면 잰다** — 스크린리더에서 숨겼는지와 무관하다. 장식 목적의 초대형 글자(ghost 넘버·인용부호 글리프·워터마크)를 배경에 가깝게 칠하면 그 자체로 하드페일이다. 하한은 크기가 정한다(24px 이상 = 대형 텍스트 3:1).
  **정본과 계측은 "보이는 텍스트"의 정의를 공유하지 않는다** — 이 방향(계측이 옳고 정본이 모름)과 반대 방향(계측이 과하게 셈)이 한 주 사이에 각각 났다. 반대 사례: `weights` 게이트가 `sr-only` 테이블의 `<th>` UA 기본 bold 를 세어 `/dash/d48` 을 4종으로 보고했다 — **차트에 표 대체를 제공한 작품일수록 위반처럼 읽힌다**([[questions-queue]] Q34·Q43). 장식/보조 텍스트를 쓸 때는 **어느 계측이 그것을 어떻게 보는지**를 먼저 확인한다.
