...
**5작품을 먼저 고치고, 그 다음 규칙을 올렸다.** 순서가 핵심이다 — 규칙을 먼저 세우면 기존 카탈로그가 깨지고, 그러면 규칙을 끄거나 예외를 파게 된다.

| 작품 | 감사 | 실제 결함 | 조치 |
|---|---|---|---|
| `dv1` | `heading-order` | 엔드포인트 헤딩이 `h3`인데 감싸는 헤딩이 페이지 `h1` | `h2` |
| `ig1` | `definition-list` | `dl > div` 안에 `<p>` (자식은 `dt`·`dd` 뿐) | 두 번째 `dd` |
| `pf1` | `target-size` | 막대 24개가 390px에서 **11.5px** (최소 24×24) | 바닥 24px + 좁은 폭에서만 가로 흐름 |
| `d29`·`d33` | `skip-link` | 사이드바 앵커가 **존재하지 않는 id**를 가리킴 — 클릭이 아무 일도 안 한다 | `#main-content` (d35가 이미 쓰던 관례) |

`d29`·`d33`의 것은 접근성 결함이면서 **그냥 깨진 링크**였다. 정본 조항에 계측을 붙이면 규칙 위반만 잡히는 게 아니라 이런 것이 딸려 나온다.

**게이트 변경**: `A11Y_HARD_AUDITS` 네 개는 **점수와 무관하게** 하드페일. 반증 — `ig1`에 결함을 되심으니 **96점인데 실패**(이전엔 통과), 복원하니 통과.

**아직 기록전용**: `color-contrast` 6 · `button-name` 5 · `label-content-name-mismatch` 3. 규칙이 달라서가 아니라 **아직 0으로 못 내려서**다.
