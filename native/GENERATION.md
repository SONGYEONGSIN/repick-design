...
- **하단 고정 밴드는 개수가 아니라 그 밴드가 일을 하는지로 판정한다.** 유효한 밴드는 (a) 지금 왜 진행 불가한지 **문장으로 말하고** (b) 눌리면 **다음 미완료 지점으로 이동**시킨다 — 즉 상태기계다. 비활성 버튼을 놓아두고 사용자가 이유를 찾게 하는 밴드는 감점 대상이고, 밴드가 아예 없는 것도 유효한 선택이다(할 일이 없으면 자리를 비운다).
  ...
  **밴드의 형태는 그 화면이 무엇을 하는 곳인지가 정한다 (2026-08-26 L2 편입).** 상태기계는 **막힌 워크플로**의 형태이지 밴드의 유일한 형태가 아니다. 두 경계가 다른 라운드에서 각각 확인됐다:
  ...
  - **선택 개수에서 파생되는 컨텍스추얼 바** — 선택 0 에서 **존재하지 않고**, 1 이상에서 등장하며 **라이브 리전이 개수를 알린다.** 사후 undo 오버레이와 **상호배타**라 화면 하단이 두 제안을 동시에 내놓지 않는다. 불리언 토글이 아니라 `selectedCount > 0` 파생 상태라는 점이 핵심이다(`r16/b`). 세 번째 형태다 — 막힌 워크플로(상태기계)도, 완료 기록(상시 액션바)도 아니다.

  **형태를 맞게 골라도 코드 표면까지 닮으면 진다 (2026-09-12 L2 편입).** 화면유형 차별성 렌즈가 "이 재사용이 정당한가"를 판정할 때 실제로 붙잡는 신호는 **원리 재사용 여부가 아니라 코드 표면의 문자적 일치**다 — 스타일 키 이름 집합(`bandBlocked`/`bandBlockedTitle`/`bandBlockedHint`/`bandReady`/…)이 기존 화면과 키 단위로 겹치거나 힌트 문구가 거의 같으면 재탕으로 읽는다(`r18/c`). 같은 축의 다른 얼굴이 `r19/b` 다 — 밴드 형태3(선택 구동 컨텍스추얼 바)을 **이미 카탈로그에 있는 화면과 구조적으로 동일**하게 만들면, 도메인만 바꾼 것으로는 차별성이 안 된다.
  **그러니 형태 선택과 구현을 분리한다** — 어떤 밴드인지는 위 규칙이 정하고, **키 이름·문구·제어흐름은 그 화면의 도메인에서 새로 짠다.** 두 라운드 독립 재현.

  > 관용구를 **재사용할 자리와 재사용하면 안 되는 자리**를 가르는 것이 이 조항의 일이다. [[questions-queue]] Q25(고정 밴드 delta 의 반증 — 화면유형이 가르는가)가 묻던 것이고, 세 라운드가 각각 "가른다" 쪽 증거를 냈다(`r12` read-only · `r13` 파괴적 확인 · `r16` 선택 파생).
...
- **`accessible={true}` 를 래퍼에 걸지 마라 — 하위 트리가 통째로 잠긴다 (2026-09-20 L2 편입).** RN 의 이 불리언을 `View`/`Pressable` 래퍼에 걸면 **그 아래 전체가 하나의 불투명한 포커스 단위**가 되어 VoiceOver·TalkBack 에 부모의 `accessibilityLabel` 하나만 노출된다. 안에 중첩된 `Pressable` 은 **라벨이 부실한 정도가 아니라 보조기술에서 구조적으로 도달 불가**가 된다.
  `r21/b`(Compare Items)가 열 헤더 셀에 이걸 걸어 그 안의 정렬 컨트롤이 통째로 사라졌다. **기계로 확인 가능한 분기라 2라운드 재현을 기다리지 않고 L2 로 올렸다**([[curation-criteria]] L2 체크리스트 — `r10` 의 accent 조항과 같은 근거). 묶어서 읽히길 원하면 `accessibilityLabel` 을 **잎 노드에** 달고 래퍼는 그대로 둔다.
- **파괴적 확인의 밴드→행 일반화 (2026-09-20 L2 편입).** `r13/a` 가 세운 관용구(별도 `Alert` 대신 **확인하는 표면 자신**을 `Cancel`/`Confirm` 두 버튼 상태로 전환, 이미 라이브 리전이 걸린 컨테이너 안에서)는 **밴드에만 걸리는 규칙이 아니다.** 전역 게이트가 자연스럽지 않은 화면(목록의 각 행이 독립적으로 파괴적인 경우)에서는 **행 단위로** 같은 전환을 쓴다 — `r22/b`(Active Sessions)가 고정 밴드를 의도적으로 두지 않고 행마다 인라인 확인으로 처리했다. **규칙이 붙는 자리는 "밴드"가 아니라 "확인이 필요한 표면"이다.**
