...
test('filesForRoute — `/` 는 자식 라우트도 그룹도 자기 스코프로 삼지 않는다', () => {
  // 같은 함수의 반대 방향 결함. `/` 는 ENOENT 로 죽지 않는 대신 **앱 트리 491 파일**을 자기
  // 스코프로 반환했다(자식 라우트 `page.tsx` 64개 포함). 그러면 다른 작품의 위반이 `/` 의 위반으로
  // 보고되고, 작품 단위 규칙(웨이트·활자 face group)은 491 파일을 한 작품으로 세어 언제나 실패한다.
  // 라우트 디렉토리는 `page.tsx` 를 가진 디렉토리다. 그 아래 또 `page.tsx` 가 있으면 자식 라우트고,
  // `(group)` 안의 파일은 그 그룹의 라우트들에 속한다 — 둘 다 별개의 검사 단위다.
  // 2026-08-27 에 갤러리가 `/` 로 오면서 `/` 의 라우트 디렉토리가 `(marketing)` 에서 앱 루트로
  // 바뀌었고, 그래서 이 단언은 "어느 디렉토리인가"가 아니라 **경계가 지켜지는가**를 잰다.
  const files = filesForRoute('/');
  assert.deepEqual(files.sort(), ['app/src/app/layout.tsx', 'app/src/app/page.tsx']);
});
